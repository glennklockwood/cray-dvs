...
/*
 * Legacy behavior of dvsdebug_stat_control: If set to 0 as module option,
 * statistics could not be turned on. This has been fixed, AND the feature
 * has been deprecated in favor of the more flexible dvsdebug_stat_defaults.
 *
 * The new dvsdebug_stat_defaults sets the default values at module load
 * time. Every new stats file will take these defaults, but can be changed
 * at any later time for each stats file.
 */
unsigned int dvsdebug_stat_control = 1;
static char *dvsdebug_stat_defaults = "enable,legacy";

module_param(dvsdebug_stat_control, int, 0644);
module_param(dvsdebug_stat_defaults, charp, 0644);

MODULE_PARM_DESC(dvsdebug_stat_control,
		 "0=disable,1=enable "
		 "(deprecated, use dvsdebug_stat_defaults)");
MODULE_PARM_DESC(dvsdebug_stat_defaults, "enable,disable,legacy,flat,json,"
 ...
 */
struct dvsdebug_stat aggregate_stats;
#define GETSTATS(stats) ((stats) ? (stats) : &aggregate_stats)

/*
 * Get the counters from the stats object, or NULL if stats are disabled.
 */
static struct dvsdebug_stat_counters *
dvsdebug_stat_get_counters(struct dvsdebug_stat *stats)
{
	 ...
	 * enabled if dvsdebug_stat_control was set to zero.
	 */
	if (stats->control) {
...
static inline void _update_len(atomic64_t *ary, ssize_t value)
{
...
static inline void _update_off(atomic64_t *ary, ssize_t value)
{
 ...
 * and tag.
 */
void dvsdebug_stat_update(struct dvsdebug_stat *stats, unsigned int type,
			  unsigned int tag, ssize_t value)
{
	struct dvsdebug_stat_counters *counters;

	counters = dvsdebug_stat_get_counters(stats);
	if (!counters) {
	...
	case DVSSYS_STAT_REQ:
		 ...
		 */
		if (tag < DVSSYS_STAT_REQ_COUNTERS) {
	...
	case DVSSYS_STAT_IO:
		...
		case RQ_READPAGE_ASYNC: /* (srv agg) dvs_rq_readpage_async()
					   (deprecated) */
		case RQ_READPAGE_DATA: /* (cli mnt) dvs_rq_readpage_data()
					  (deprecated) */
			/*
			 * Set maxval first, so that maxval >= minval is always
			 * true.
			 */
	...
	case DVSSYS_STAT_IPC_REQUEST:
		...
		break;
	case DVSSYS_STAT_IPC_REQUEST_ASYNC:
		/*
	...
	case DVSSYS_STAT_OPEN_FILES:
		...
		break;
	case DVSSYS_STAT_OPER:
		/*
		 * uwhatever_stats
		 */
		if (tag < DVSSYS_STAT_OPER_COUNTERS) {
			if (value < 0) /* update failure */
		...
		break;
	case DVSSYS_STAT_OPER_TIME:
		/*
		 * uwhatever_stats
		 */
		if (tag < DVSSYS_STAT_OPER_COUNTERS) {
			/*
			 * Set max time first so that dvsdebug_stat_print()
		...
		break;
	case DVSSYS_STAT_REQP:
		/*
		 * receipt of IPC request from remote node
		 */
		if (tag < DVSSYS_STAT_REQP_COUNTERS) {
			if (value < 0) /* update failure */
	...
	case DVSSYS_STAT_CLIENT_OFF:
		...
		break;
	case DVSSYS_STAT_CREATE:
		switch (tag) {
		case DVSSYS_STAT_TYPE_INODE:
			atomic64_inc(&counters->inodes_created);
			break;
		case DVSSYS_STAT_TYPE_FILE:
			atomic64_inc(&counters->files_created);
			break;
		case DVSSYS_STAT_TYPE_SYMLINK:
			atomic64_inc(&counters->links_created);
			break;
		case DVSSYS_STAT_TYPE_DIRECTORY:
			atomic64_inc(&counters->dirs_created);
			break;
		}
		break;
	case DVSSYS_STAT_DELETE:
		switch (tag) {
		case DVSSYS_STAT_TYPE_INODE:
 ...
 */
void dvsdebug_stat_print_legacy(struct seq_file *m, struct dvsdebug_stat *stats)
{
	int i;
	struct dvsdebug_stat_counters *counters;
	...
	for (i = 0; i < DVSSYS_STAT_REQ_COUNTERS; i++) {
		...
		seq_printf(m, "%s: %lu %lu %lu %lu %ld.%03ld %ld.%03ld\n",
			   ...
			   time_last / 1000, time_last % 1000, time_max / 1000,
			   time_max % 1000);
	}
	for (i = 0; i < DVSSYS_STAT_OPER_COUNTERS; i++) {
		time_last = jiffies_to_msecs(
 ...
 * of control options.
 */
static const char *_stat_flags_to_string(char *buf, int siz,
					 struct dvsdebug_stat *stats)
{
	...
	const char *tst;
	switch (stats->format) {
	case DVSSYS_STAT_FMT_LEGACY:
		fmt = "legacy";
		break;
	case DVSSYS_STAT_FMT_HELP:
		fmt = "help";
		break;
	case DVSSYS_STAT_FMT_FLAT:
		fmt = "flat";
		break;
	case DVSSYS_STAT_FMT_JSON:
		fmt = "json";
		break;
	default:
		fmt = "unknown";
	...
	vrb = (stats->fflags & DVSSYS_STAT_FFLG_VERBOSE) ? "verbose" : "brief";
	prt = (stats->fflags & DVSSYS_STAT_FFLG_PRETTY) ? "pretty" : "plain";
	tst = (stats->fflags & DVSSYS_STAT_FFLG_TEST) ? "test" : "notest";
 ...
 */
static inline const char *_json_colon(statnode_t *snp)
{
	return (snp->pretty) ? ": " : ":";
}
 ...
 */
static inline const char *_json_LF(statnode_t *snp, const char *pfx)
{
	char *lf;
	int spc;
 ...
 */
static inline const char *_json_comma(statnode_t *snp)
{
	char *comma = "";
 ...
 */
static inline void _stat_clear(statnode_t *snp)
{
	snp->tags[0] = 0;
	snp->depth = 0;
	snp->flushed = 0;
 ...
 */
static void _stat_init(statnode_t *snp, struct seq_file *m,
		       struct dvsdebug_stat *stats)
{
	_stat_clear(snp);
	snp->format = stats->format;
	snp->pretty = (stats->fflags & DVSSYS_STAT_FFLG_PRETTY) != 0;
	snp->m = m;
}
 ...
 */
static void _stat_push(statnode_t *snp, const char *tag)
{
	const char *fmt = "%s";
 ...
 */
static void _stat_pop(statnode_t *snp)
{
	/*
 ...
 */
static void _stat_flush(statnode_t *snp)
{
	const char *tag;
	const char *sep;
	int depth = snp->depth;
	...
	case DVSSYS_STAT_FMT_JSON:
		...
		while (snp->depth < depth) {
			...
			seq_printf(snp->m, "%s%s%s{", _json_comma(snp), tag,
				   sep);
 ...
 */
static inline unsigned long _divisor(int places)
{
	unsigned long div = 1;
	while (places-- > 0)
		div *= 10;
 ...
 */
static void _stat_prnum(statnode_t *snp, const char *tag, unsigned long value,
			int places)
{
	...
	case DVSSYS_STAT_FMT_JSON:
		...
		_stat_flush(snp);
		if (!places) {
			seq_printf(snp->m, "%s\"%s\"%s%lu", _json_comma(snp),
				   tag, _json_colon(snp), value);
		} else {
			seq_printf(snp->m, "%s\"%s\"%s%lu.%0*lu",
				   _json_comma(snp), tag, _json_colon(snp),
 ...
 */
static void _stat_prstr(statnode_t *snp, const char *tag, const char *value)
{
	switch (snp->format) {
	case DVSSYS_STAT_FMT_FLAT:
		_stat_push(snp, tag);
	...
	case DVSSYS_STAT_FMT_JSON:
		_stat_flush(snp);
		seq_printf(snp->m, "%s\"%s\"%s\"%s\"", _json_comma(snp), tag,
 ...
 */
static void _stat_term(statnode_t *snp)
{
	switch (snp->format) {
	case DVSSYS_STAT_FMT_FLAT:
		/* nothing to do */
 ...
 */
void dvsdebug_stat_print_test(struct seq_file *m, struct dvsdebug_stat *stats)
{
	statnode_t sn;
	int verbose = (stats->fflags & DVSSYS_STAT_FFLG_VERBOSE) != 0;
 ...
 */
void dvsdebug_stat_print_help(struct seq_file *m, struct dvsdebug_stat *stats)
{
	static char *help =
 ...
 */
static void _compute_rate(atomic64_t *data, unsigned long *iops,
			  unsigned long *bytes)
{
 ...
 */
void dvsdebug_stat_print_data(struct seq_file *m, struct dvsdebug_stat *stats)
{
...
#define POP()                                                                  \
	...
	if (!(counters = dvsdebug_stat_get_counters(stats)))
		goto done;
	...
	PSH("RQ")
	{
		for (i = 0; i < DVSSYS_STAT_REQ_COUNTERS; i++) {
			...
			PSH(tag)
			{
				...
				switch (i) {
				case RQ_PARALLEL_READ: /* (srv agg, cli mnt)
							  sync read */
				case RQ_READPAGES_RQ: /* (srv agg) page read */
				case RQ_READPAGES_RP: /* unused */
				case RQ_READPAGE_ASYNC: /* (srv agg)
							   dvs_rq_readpage_async()
							   (deprecated) */
				case RQ_READPAGE_DATA: /* (cli mnt)
					...
					break;
				case RQ_PARALLEL_WRITE: /* (srv agg, cli mnt)
							   sync write */
					bmin = atomic64_read(
						&counters->write_min);
					bmax = atomic64_read(
		...
		for (i = 0; i < DVSSYS_STAT_OPER_COUNTERS; i++) {
			...
			PSH(tag)
			{
				...
				if (verbose || good || fail) {
					...
					PSH("dur")
					{
						_stat_prnum(&sn, "prv", tprv,
							    3);
						_stat_prnum(&sn, "max", tmax,
							    3);
					}
					POP()
				}
	...
	PSH("PERF"){ PSH("user"){
		PSH("read"){ atomic64_t *ary = counters->user_read_stats;
...
POP()
PSH("cache"){ PSH("read"){ atomic64_t *ary = counters->page_read_stats;
bmin = atomic64_read(&ary[0]);
bmax = atomic64_read(&ary[1]);
if (verbose || bmin || bmax) {
	_stat_prnum(&sn, "min_len", bmin, 0);
...
POP()
PSH("legacy"){ PSH("inodes"){ bmin = atomic64_read(&counters->inodes_created);
if (verbose || bmin)
 ...
 */
void dvsdebug_stat_print(struct seq_file *m, struct dvsdebug_stat *stats)
{
	stats = GETSTATS(stats);
	if (stats->fflags & DVSSYS_STAT_FFLG_TEST) {
		dvsdebug_stat_print_test(m, stats);
		return;
	}
	switch (stats->format) {
	case DVSSYS_STAT_FMT_LEGACY:
		dvsdebug_stat_print_legacy(m, stats);
		break;
	case DVSSYS_STAT_FMT_HELP:
		dvsdebug_stat_print_help(m, stats);
		break;
	case DVSSYS_STAT_FMT_FLAT:
	case DVSSYS_STAT_FMT_JSON:
		dvsdebug_stat_print_data(m, stats);
		break;
	}
}
 ...
 */
void dvsdebug_stat_counters_reset(struct dvsdebug_stat_counters *counters)
{
	atomic64_t open_files_tmp;

	open_files_tmp = counters->open_files;
	memset(counters, 0, sizeof(struct dvsdebug_stat_counters));
 ...
 */
static inline int _charin(char c, char *s)
{
	while (*s)
		if (c == *s++)
			return 1;
 ...
 */
int dvsdebug_stat_set_control(struct dvsdebug_stat *stats, char *control)
{
	char *ptr, *key;
	char local[32];
	int value;
	...
	if (*ptr == 0) {
		...
		case 3:
			...
			break;
		default:
			printk(KERN_ERR "DVS: invalid control code '%s'"
					" for mount %d\n",
	...
	while (*ptr) {
		...
		if (!strcmp(key, "disable")) {
			...
			stats->fflags |= DVSSYS_STAT_FFLG_VERBOSE;
			...
			stats->fflags |= DVSSYS_STAT_FFLG_PRETTY;
			...
			printk(KERN_ERR "DVS: invalid control directive '%s'"
					" for mount %d\n",
 ...
 * Start up the module.
 */
void dvsdebug_stat_init(struct dvsdebug_stat *stats, int mountpoint_id)
{
	...
	stats->magic = DVSSYS_STAT_MAGIC;
	/* version is a fixed number for this compile */
	stats->version = DVSSYS_STAT_VERSION;
	/* deprecated: use this to pre-load the default state */
	stats->control = dvsdebug_stat_control;
	/* make a writable copy of the module parameter */
	snprintf(defaults, sizeof(defaults), "%s", dvsdebug_stat_defaults);
	/* parse the writable copy */
	dvsdebug_stat_set_control(stats, defaults);
	/* always reset the counters */
	dvsdebug_stat_counters_reset(&stats->counters);
}

EXPORT_SYMBOL(dvsdebug_stat_update);
