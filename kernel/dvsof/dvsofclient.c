...
void (*dvs_task_work_run_func)(void) = NULL;
rwlock_t *dvs_tasklist_lock = NULL;

/* forward declarations */
static int urelease_common(struct inode *ip, struct file *fp, int retry);
static int uopen(struct inode *ip, struct file *fp, int retrying);
...
int create_dwfs_data_paths(struct file *fp, struct dwfs_open_info *dwfs_info)
{
	...
	/*
	 * Change the file's stripe width and size based on what we received
	 * from the server.
	 */
	if (dwfs_info->stripe_width > FILE_PRIVATE(fp)->data_rf_len) {
		printk(KERN_ERR "DVS: dwfs file %p invalid stripe width\n", fp);
		return -EINVAL;
	}
	FILE_PRIVATE(fp)->data_rf_len = dwfs_info->stripe_width;
	FILE_PRIVATE(fp)->blocksize = dwfs_info->stripe_size;
	KDEBUG_OFC(0, "DVS: file %p stripe_width %d stripe_size %d\n", fp,
		   FILE_PRIVATE(fp)->data_rf_len, FILE_PRIVATE(fp)->blocksize);
	...
	size = snprintf(buf, 1, "/%s/%s%d", FILE_ICSB(fp)->remoteprefix,
			stripe_path, INODE_ICSB(inode)->data_servers_len - 1);

	bcstripe_size =
		snprintf(buf, 1, "/%s/%s%d", FILE_ICSB(fp)->remoteprefix,
			 bcstripe_path, bcstripe_index);
	...
	for (i = 0; i < FILE_PRIVATE(fp)->data_rf_len; i++) {
		if ((start_nord + i) % INODE_ICSB(inode)->data_servers_len ==
		    bcstripe_index)
			path = bcstripe_path;
		else
			path = stripe_path;
		...
		KDEBUG_OFC(0,
			   "file %p nord %d data_rf_len %d stripe path: %s\n",
			   fp, i, FILE_PRIVATE(fp)->data_rf_len, path);

		if (size <
		    snprintf(data_path + (size * i), size, "/%s/%s%d",
			     FILE_ICSB(fp)->remoteprefix, path,
			     (start_nord + i) %
				     INODE_ICSB(inode)->data_servers_len)) {
			kfree_ssi(data_path);
			printk(KERN_ERR
			       "DVS: %s Setting dwfs_data_path to NULL\n",
			       __func__);
			...
			printk("DVS: Error: DWFS data path for remote_prefix "
			       ...
			       "length of %lu\n",
			       FILE_ICSB(fp)->remoteprefix, stripe_path,
			       (start_nord + i) %
				       INODE_ICSB(inode)->data_servers_len,
			       size);
			return -ENAMETOOLONG;
...
void mark_remote_file_quiesced(struct file *fp, struct remote_file *rf)
{
	...
	int rf_offset =
		(rf - FILE_PRIVATE(fp)->rf) / sizeof(struct remote_file);
	char pb[32] = "";
	...
	KDEBUG_QSC(0, "Quiescing remote file %s:%d FILE_NNODES %d\n",
		   dvs_dentry_path(fp->f_path.dentry, pb, sizeof(pb)),
	...
	if (!list_empty(&rf->list)) {
		 ...
		 */
		DVS_BUG_ON(atomic_dec_return(&server_info->open_files) < 0);
	} else {
...
int file_ops_retry(struct file *fp, char *opname, int orig_rval)
{
	...
	if (orig_rval == -EQUIESCE) {
		...
		KDEBUG_QSC(
			0,
			"Op %s file %p No unquiesced remote files FILE_NNODES"
			" %d\n",
	...
	while (1) {
		...
		if (sleep_before_reopen) {
			...
			KDEBUG_OFC(
				0,
				"DVS: file_ops_retry: %s: begin delay (%d seconds) "
				"before retry %d, ip=0x%p, fp=0x%p, path=%s\n",
				opname, RETRYSLEEP, retry, ip, fp,
				path ? path : "N/A");
			...
			if (schedule_timeout(RETRYSLEEP * HZ)) {
				...
				KDEBUG_OFC(
					0,
					"DVS: file_ops_retry: %s: retry delay "
					"interrupted, bailing out, ip=0x%p, fp=0x%p, "
					"path=%s\n",
					opname, ip, fp, path ? path : "N/A");
				rval = orig_rval;
				if (!retryalready) {
					down(&FILE_PRIVATE(fp)->rip_sema);
		...
		KDEBUG_OFC(0,
			   "DVS: file_ops_retry: %s: begin retry %d, ip=0x%p, "
			   "fp=0x%p path=%s\n",
			   opname, retry, ip, fp, path ? path : "N/A");
		...
		if (orig_rval == -EQUIESCE) {
			KDEBUG_QSC(0,
				   "Op %s Attempting to reopen file %s due to "
				   "quiesce\n",
				   opname,
				   dvs_dentry_path(fp->f_path.dentry, pb,
						   sizeof(pb)));
		}

		if ((oval = uopen(ip, fp, 1 /* we're retrying */)) < 0) {
			/* retry re-open for "node down" only. uopen can't
			 * return -ESTALE_DVS_RETRY. */
			if (orig_rval == -EQUIESCE) {
				KDEBUG_QSC(0,
					   "Op %s Reopen of %s due to "
					   "quiesce returned %d\n",
					   opname,
					   dvs_dentry_path(fp->f_path.dentry,
							   pb, sizeof(pb)),
					   oval);
			}
		...
		KDEBUG_OFC(0,
			   "DVS: file_ops_retry: %s: retry %d re-open "
...
int file_node_estale(struct inode *ip, struct file *fp, int node)
{
	...
	if (!finfo->estale_nodes) {
		...
		if ((buf = kmalloc_ssi(sizeof(char) * MAX_PFS_NODES,
				       GFP_KERNEL)) == NULL)
			return -ESTALE;
	...
	if (finfo->estale_num_nodes >= finfo->estale_max_nodes - 1) {
		...
		ESTALE_LOG(
			"ESTALE: fp 0x%p (file: %s) - Exhausted all servers. Returning "
			"ESTALE\n",
			fp, fpname(fp));
 ...
 */
int send_ipc_file_retry(char *myname, struct file *fp,
			struct remote_file *rf_array, int rf_len, int nord,
			struct file_request *filerq, int rqsz,
			struct file_reply *freply, int rpsz, int *node)
{
	int rval = 0, last_rval = 0, rval2 = 0, retry = 0, nodetotry,
	    estale_retry = 0;
	...
	KDEBUG_OFC(
		0,
		"DVS-send_ipc_file_retry called fp %p rf_array %p rf_len %d\n",
		fp, rf_array, rf_len);
	...
	while (1) {
		...
		if (rval == -EQUIESCE && filerq->request == RQ_OPEN) {
			...
			KDEBUG_QSC(
				0,
				"Op %s Node %s path %s was quiesced for node_offset %d, "
				"servers_len %d\n",
				myname, SSI_NODE_NAME(nodetotry),
				filerq->u.openrq.pathname, node_offset,
				servers_len);
			node_offset++;
			if (node_offset == servers_len) {
				printk(KERN_ERR
				       "DVS: send_ipc_file_retry: "
			...
			KDEBUG_QSC(
				0,
				"Op %s Node %s file %p was quiesced for rf_offset %d, "
				"data_servers_len %d\n",
				myname, SSI_NODE_NAME(nodetotry), fp, rf_offset,
				INODE_ICSB(ip)->data_servers_len);
		...
		if (filerq->request != RQ_OPEN &&
		    rf_array[rf_offset].quiesced) {
			KDEBUG_QSC(0,
				   "Op %s file %p rf_offset %d quiesced! "
				   "FILE_NNODES: %d\n",
				   myname, fp, rf_offset, rf_len);
			rval = -EQUIESCE;
			continue;
		}
		...
		if (filerq->request != RQ_OPEN) {
			...
			if (rf_array[rf_offset].rf_type & RF_TYPE_META)
				nodetotry = inode_meta_server(
					ip, rf_offset + node_offset);
			else
				nodetotry = inode_data_server(
					ip, rf_offset + node_offset);
		}
		/*
		...
		*node = nodetotry;
		...
		KDEBUG_OFC(0,
			   "DVS: set filerq dwcfs mds node %d FILE_NODE %d\n",
			   filerq->dwcfs_mds, nodetotry);

		identity = filerq->request == RQ_OPEN ?
				   NO_IDENTITY :
				   rf_array[rf_offset].identity;
		RESET_FILERQ(filerq);
		if (freply == NULL) {
			rval = send_ipc_request_async_stats(
				INODE_ICSB(ip)->stats, nodetotry, RQ_FILE,
				filerq, rqsz, NULL, 0, identity);
		} else {
		...
		if (rval >= 0) {
			if (last_rval == -EQUIESCE) {
				KDEBUG_QSC(
					0,
					"Op %s request succeeded for node %s, %s %d\n",
					myname, SSI_NODE_NAME(nodetotry),
		...
		if (filerq->request == RQ_OPEN) {
			if (rval == -ESTALE_DVS_RETRY) {
				...
				if (estale_retry >= estale_max_retry) {
					 ...
					 * inode_ops_retry() since we don't need
					 * a delay. We know we're switching to a
					 * different server since this one is
					 * ESTALE. */
					continue;
				}

				ESTALE_LOG(
					"ESTALE: fp 0x%p (file: %s) - Retrying "
					"operation on original server %s\n",
					fp, fpname(fp),
					SSI_NODE_NAME(nodetotry));
			}

			if ((rval2 = inode_ops_retry(ip, fp->f_path.dentry,
						     myname, retry, rval,
						     nodetotry)) < 0) {
				return (rval2);
			}
	...
	if (estale_retry && filerq->flags.estale_failover) {
		ESTALE_LOG("ESTALE: fp 0x%p (file: %s) - Failover to server %s "
			   "succeeded\n",
			   fp, fpname(fp), SSI_NODE_NAME(nodetotry));
	}
	if (freply)
 ...
 */
int send_multi_async_ipc_file_retry(char *myname, struct file *fp,
				    struct remote_file *rf_array, int rf_len,
				    struct file_request *filerq, int filerq_sz,
				    int filerp_sz, struct per_node **caller_pna,
				    int *node_used)
{
	...
	KDEBUG_OFC(0,
		   "DVS: send_multi_async_ipc_file_retry: %s:\tfp=0x%p "
	...
	for (nord = 0; nord < rf_len; nord++) {
		...
		if (rf->rf_type == 0) {
			printk(KERN_ERR
			       "DVS: Op %s fp %p rf[%d]->rf_type is 0!\n",
			       myname, fp, nord);
		}

		frp = pna[nord].reply =
			(struct file_reply *)kmalloc_ssi(filerp_sz, GFP_KERNEL);
		if (!frp) {
			error = -ENOMEM;
			break;
		}
	...
	send_multi_async_ipc_file_retry:
		...
		if (frq->request == RQ_OPEN) {
			...
			if (!rf->valid || rf->quiesced) {
				if (rf->quiesced) {
					KDEBUG_QSC(
						0,
						"Op %s File %p nord %d FILE_NNODES %d "
		...
		RESET_FILERQ(frq);
		rval = send_ipc_request_async_stats(INODE_ICSB(ip)->stats, node,
						    RQ_FILE, frq, filerq_sz,
						    frp, filerp_sz, identity);
		if (rval < 0) {
			/*
			...
			if (filerq->request == RQ_OPEN) {
				rval = inode_ops_retry(ip, fp->f_path.dentry,
						       myname, retry_count,
						       rval, node);
			} else {
			...
			if (rval < 0) {
				KDEBUG_OFC(
					0,
					"DVS: send_multi_async_ipc_file_retry: "
		...
		if (retry_count) {
			printk(KERN_INFO "DVS: send_multi_async_ipc_file_retry:"
					 " %s: retry %d OK, node %s\n",
			       myname, retry_count, SSI_NODE_NAME(node));
		}
	}
	...
	for (nord = 0; nord < rf_len; nord++) {
		...
		DVS_TRACEL("smaifrW", nord, pna[nord].request, pna[nord].reply,
			   0, 0);
		...
		if (rval < 0) {
			...
			/*
			 * Retry for "node down", ESTALE and EQUIESCE retry
			 * only. Everything else causes the entire operation to
			 * fail. No retries for RQ_CLOSE either (node down
			 * implies file closed).
			 */
			if ((rval != -EHOSTDOWN && rval != -ESTALE_DVS_RETRY &&
			     rval != -EQUIESCE) ||
				...
				printk(KERN_ERR
				       ...
				       "(rval=%d) %s\n",
				       myname, SSI_NODE_NAME(node), rval,
			...
			if (rval == -EQUIESCE && frq->request != RQ_OPEN) {
				KDEBUG_QSC(
					0,
					"Op %s File %p nord %d returned "
					"EQUIESCE. Calling send_ipc_file_retry\n",
					myname, fp, nord);
				mark_remote_file_quiesced(fp, &rf_array[nord]);
			}
			if ((rval = send_ipc_file_retry(
					    myname, fp, rf_array, rf_len, nord,
					    frq, filerq_sz, frp, filerp_sz,
					    &node) < 0)) {
				KDEBUG_OFC(
					0,
					"DVS: send_multi_async_ipc_file_retry: "
	...
	if (!error)
		dvs_update_client_bulk_stats(pna, rf_len);
...
static inline int async_op_valid(struct async_retry *p, int node)
{
...
void __async_op_retry(int node, int shutdown, struct file_request *filerq)
{
	 ...
	 * in the loop below to deal with them.   We mark them to hold them
	 * since the below loop drops/reacquires the lock a bunch of times.
	 */
	down(&aretrysem[node]);
	list_for_each_safe (lp, tp, &alist[node]) {
		...
		if (handle_estale) {
			if (p->filerq == filerq) {
				/*
				 * We know if we're in this routine for the
				 * first time or the previous attempts didn't
				 * work, so regardless of this one's state,
				 * we're going to retry it.
				 */
...
restartlist:
	...
	list_for_each_safe (lp, tp, &alist[node]) {
		...
		if (!shutdown &&
		    (!handle_estale && p->ar_status != AR_Will_Retry)) {
		...
		if (shutdown || !p->filerq->retry || !p->fp) {
			...
			ip = (p->fp) ? file_inode(p->fp) :
				       p->filerq->u.iopagesrq.rq->ip;
			...
			if (node == p->ar_node) {
				...
				/*
				 * That's our node and it's down apparently.
				 * Let's retry to another server.
				 */
				if ((!identity_valid(p->ar_node,
						     DATA_RF(p->fp, p->ar_nord)
							     ->identity) &&
				     (p->ar_status == AR_Will_Retry)) ||
				    handle_estale) {
					KDEBUG_OFC(
						0,
						"DVS: %s: retrying ar 0x%p fp 0x%p node %d estale %d\n",
						__FUNCTION__, p, p->fp, node,
						handle_estale);
					p->ar_status = AR_Retrying; /* hold it
								     */
					up(&aretrysem[node]);

					rval = file_ops_retry(
						p->fp, "async_op_retry", rval);

					/*
					 * We just marked the request above
					 * (under lock) as Retrying which means
					 * it's ours and nobody else can mess
					 * with it.
					 */
					...
					if ((rval < 0) &&
						 ...
						 * what to do in the error cases
						 * (here and below for the send)
						 *
						 * Current approach will be to
						 * retry this one for a while
						 * and if that doesn't get us
						 * anywhere, mark it as Default,
						 * mark the request as invalid
						 * and it'll be errored and
						 * cleaned up later.
						 *
						 * We own the request even
						 * though we're not holding the
						 * semaphore since it's in the
						 * Retrying state.
						 *
						 */
						if (++p->ar_retry_count <
						...
						if (!DATA_RF(p->fp, p->ar_nord)
							     ->valid ||
						    DATA_RF(p->fp, p->ar_nord)
							    ->quiesced) {
							KDEBUG_OFC(
								0,
								"%s: nord %d invalid handle - deferring open\n",
								__FUNCTION__,
								p->ar_nord);
							rval = deferred_uopen(
								file_inode(
					...
					p->filerq->file_handle =
						DATA_RF(p->fp, p->ar_nord)
							->file_handle;

					KDEBUG_OFC(
						0,
						"DVS: %s: About to send %s RETRY to node %s\n",
						__FUNCTION__, "async_op_retry",
						SSI_NODE_NAME(
							DATA_RF(p->fp,
								p->ar_nord)
								->remote_node));
					new_node = DATA_RF(p->fp, p->ar_nord)
							   ->remote_node;
					DVS_TRACEL("aorRsend", p, p->fp,
						   p->filerq, p->ar_node,
						   new_node);
					...
					if ((rval < 0) &&
					    (rval != -EHOSTDOWN)) {
						printk(KERN_ERR
						       "DVS: %s: error %d during send_async\n",
						       __FUNCTION__, rval);
						p->filerq->retry = 0;
						atomic_set(
							&p->filerq->u.iopagesrq
								 .rq->state,
							RPS_RQ_INVALID);
					}
					/*
					 * The timing logged for this request is
					 * a bit misleading as it does not cover
					 * the response.
					 */
					log_request(p->filerq->request, NULL,
						    file_inode(p->fp), p->fp, 1,
						    new_node,
						    jiffies - elapsed_jiffies);

					/*
					 * Exit if we are handling an ESTALE
					 * request (and only that request vs.
					 * the rest of the transactions that are
					 * pending for that node) and the send
					 * was successful.
					 */
					if (handle_estale && rval >= 0) {
						/*
						 * ar_status may need to be an
						 * atomic due to this next check
						 * but this should be OK since
						 * above, if the message is
						 * already returned and another
						 * thread is in this routine,
						 * they'll press ahead once
						 * they've found their message
						 * regardless of its status.  We
						 * know they'd set it to
						 * AR_Will_Retry while marking
						 * candidates but we could
						 * change that back to
						 * AR_Default below but they'll
						...
						/*
						 * The send was either
						 * successful or we had an error
						 * on the send. Mark it Default
						 * either way.
						 */
...
static loff_t ulseek(struct file *fp, loff_t off, int op)
{
	...
	elapsed_jiffies = jiffies;
	rval = send_ipc_file_retry("ulseek", fp, FILE_PRIVATE(fp)->meta_rf,
				   FILE_PRIVATE(fp)->meta_rf_len, 0, filerq,
				   sizeof(struct file_request), filerp,
				   sizeof(struct file_reply), &node);
	if (rval >= 0) {
		log_request(filerq->request, NULL, file_inode(fp), fp, 1, node,
			    jiffies - elapsed_jiffies);
	}
...
static int build_iovec(struct io_vector *iov, int nblks, int nord,
		       struct usi_iovec *target)
{
	int i, j = 0;
	...
	for (i = 0; i < nblks; i++) {
		...
		KDEBUG_OFC(0,
			   "DVS: build_iovec: nord: %d addr: 0x%p count: %ld "
			   "offset: %Ld\n",
			   iov[i].nord, iov[i].address, iov[i].count,
			   iov[i].offset);
		j++;
	}
...
static ssize_t uread2(struct file *fp, struct inode *ip, char *buf, size_t size,
		      loff_t *offp)
{
	int rval, rsz;
	struct file_request *filerq = NULL;
	...
	KDEBUG_OFC(0,
		   "DVS: uread2: size %ld nnodes %d, pos %ld, nblks %d, "
		   "blksize %d\n",
		   (long)size, nnodes, (long)pos, nblks, blksize);
	pna = (struct per_node *)kmalloc_ssi(sizeof(struct per_node) * nnodes,
					     GFP_KERNEL);
	iov = (struct io_vector *)kmalloc_ssi(sizeof(struct io_vector) * nblks,
					      GFP_KERNEL);
	if (!pna || !iov) {
		rval = -ENOMEM;
		goto out;
...
#ifndef WITH_LEGACY_CRAY
		sema_init(&rdsema, 0);
		register_ipc_read_complete(rma_handle, wait_read_rdma_done,
					   &rdsema);
#endif
	} else {
		rma_handle = NULL;
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		filerp = pna[nord].reply = kmalloc_ssi(
			sizeof(struct file_reply) + piggyback, GFP_KERNEL);
		if (!filerq || !filerp) {
			rval = -ENOMEM;
			goto done;
		}
		...
		if (rma_handle)
			build_rma_list(rma_handle,
				       (u64 *)(&filerq->u.ioprq.iov +
					       pna[nord].count));
		capture_context((&filerq->context));
		filerq->u.ioprq.count =
			build_iovec(iov, cv, nord, filerq->u.ioprq.iov);
		...
		rval = send_ipc_request_async_stats(
			...
			sizeof(struct file_reply) + piggyback,
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		rval = wait_for_async_request_stats(INODE_ICSB(ip)->stats,
						    filerq);
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		if (filerp->rval < 0) {
			KDEBUG_OFC(0,
				   "DVS: uread2: got error from server %ld\n",
				   filerp->rval);
			/* report first error */
			error = filerp->rval;
			break;
			...
			for (i = 0; i < pna[nord].count; i++) {
				...
				KDEBUG_OFC(
					0,
					"DVS: uread2: read piggyback: 0x%p %d\n",
					pna[nord].request->u.ioprq.iov[i].address,
					len);
				...
				if (tlen == 0) {
					/*
					 * The read may have been short because
					 * of a hole, and so needs to be padded
					 */
					if (len <
					    pna[nord]
						    .request->u.ioprq.iov[i]
						    .count) {
						int pad = pna[nord]
								  .request->u
								  .ioprq.iov[i]
		...
		if (nord == 0) {
			/* don't update_inode(), just take a few interesting
			 * things... */
	...
	if (error) {
		...
		*offp += xfer;
		...
		dvsdebug_stat_update(INODE_ICSB(ip)->stats, DVSSYS_STAT_IO,
				     RQ_PARALLEL_READ, xfer);
	}

done:
	for (nord = 0; nord < nnodes; nord++) {
		if (pna[nord].request && pna[nord].sent)
			wait_for_async_request_stats(INODE_ICSB(ip)->stats,
						     pna[nord].request);
	}

	dvs_update_client_bulk_stats(pna, nnodes);
...
#endif
{
	...
	if (S_ISREG(ip->i_mode) && FILE_PRIVATE(fp)->cache &&
	    !(fp->f_flags & O_DIRECT) &&
	    (!FILE_PRIVATE(fp)->cache_read_sz ||
	     (vec_len < FILE_PRIVATE(fp)->cache_read_sz))) {
		/*
		...
		KDEBUG_OFC(0,
			   "DVS: uread: generic_file_read_iter returns %ld\n",
			   tval);
#endif
		goto done;
	}
	...
	ureadretry:
		...
		while (size) {
			...
			if (rval < (ssize_t)0) {
				/* retry for "node down" and ESTALE retry and
				 * EQUIESCE only */
	...
	if (tval > 0) {
		 ...
		 * offset.
		 */
		dvsdebug_stat_update(INODE_ICSB(ip)->stats,
				     DVSSYS_STAT_CLIENT_LEN, VFS_OP_AIO_READ,
				     tval);
		dvsdebug_stat_update(INODE_ICSB(ip)->stats,
				     DVSSYS_STAT_CLIENT_OFF, VFS_OP_AIO_READ,
				     pos + tval);
	}
	return (tval);
}

static ssize_t uwrite2(struct file *fp, struct inode *ip, const char *buf,
		       size_t size, loff_t *offp)
{
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		if (rma_handle) {
			...
			KDEBUG_OFC(0,
				   "DVS: %s: rsz %d count %d piggyback %Ld\n",
				   __FUNCTION__, rsz, pna[nord].count,
				   piggyback);
			rsz += piggyback;
		}
		...
		if (rsz > 128 * 1024) {
			...
			KDEBUG_OFC(
				0,
				"DVS: %s: attempt to allocate %d bytes from kmem - returning ENXIO\n",
				__FUNCTION__, rsz);
			rval = -ENXIO;
			goto done;
		}
		...
		if (rma_handle) {
			...
			char *dp = (char *)(&filerq->u.ioprq.iov +
					    pna[nord].count);
			int i, len;
			...
			for (i = 0; i < pna[nord].count; i++) {
				...
				KDEBUG_OFC(
					0,
					"DVS: uwrite2: write piggyback: 0x%p %d\n",
					pna[nord].request->u.ioprq.iov[i].address,
					len);
				if (copy_from_user(dp,
						   pna[nord]
							   .request->u.ioprq
							   .iov[i]
							   .address,
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		if (filerp->rval < 0) {
			KDEBUG_OFC(0,
				   "DVS: uwrite2: got error from server %ld\n",
				   filerp->rval);
			/* report first error */
			if (error == 0)
				error = filerp->rval;
			...
			if (nord == 0) {
				...
				/* don't update_inode(), just take a few
				 * interesting things... */
				dvs_update_timespec(
					&ip->i_mtime,
	...
	for (nord = 0; nord < nnodes; nord++) {
		if (pna[nord].request) {
			if (pna[nord].sent && (wait_for_async_request_stats(
						       INODE_ICSB(ip)->stats,
						       pna[nord].request) == 0))
				if (pna[nord].reply &&
				    pna[nord].reply->rval >= 0)
					sync_client_data_written(
						DATA_RF(fp, nord));
		}
	}
...
#endif
{
	...
	if (S_ISREG(ip->i_mode) && FILE_PRIVATE(fp)->cache &&
		...
		KDEBUG_OFC(0,
			   "DVS: uread: generic_file_aio_write returns %ld\n",
			   rval);
#else
		...
		KDEBUG_OFC(0,
			   "DVS: uread: generic_file_write_iter returns %ld\n",
			   rval);
#endif
	...
	uwriteretry:
		...
		while (size) {
			...
			KDEBUG_OFC(
				0,
				"DVS: uwrite: 0x%p %ld %ld %ld %ld %Ld %Ld\n",
				buf, mxlen, size, rval, tval, off, fp->f_pos);
			...
			if (rval < (ssize_t)0) {
				...
				if (rval != (ssize_t)-EHOSTDOWN &&
				    ...
				    rval != (ssize_t)-EQUIESCE)
					break;
				if ((rval = file_ops_retry(fp, "uwrite",
							   rval)) < 0) {
					break;
				}
	...
	if (tval > 0) {
		 ...
		 */
		dvsdebug_stat_update(INODE_ICSB(ip)->stats,
				     DVSSYS_STAT_CLIENT_LEN, VFS_OP_AIO_WRITE,
				     tval);
		dvsdebug_stat_update(INODE_ICSB(ip)->stats,
				     DVSSYS_STAT_CLIENT_OFF, VFS_OP_AIO_WRITE,
				     pos + tval);
	}
	return (tval);
}
...
static int ureaddir(struct file *fp, struct dir_context *ctx)
{
	...
	elapsed_jiffies = jiffies;
	rval = send_ipc_file_retry(
		"ureaddir", fp, FILE_PRIVATE(fp)->meta_rf,
		FILE_PRIVATE(fp)->meta_rf_len, 0, filerq,
		sizeof(struct file_request), filerp,
		sizeof(struct file_reply) + filerq->u.readdirrq.count, &node);
	if (rval >= 0)
...
static long do_remote_ioctl(struct file *fp, struct ioctl_desc *id,
			    void *arg_val)
{
	int rval = 0, node;
	struct file_request *filerq = NULL;
	...
	if (id->cmd == DVS_BCAST_IOCTL ||
	    id->cmd == DVS_AUGMENTED_BCAST_IOCTL) {
		...
		if ((rval = send_multi_async_ipc_file_retry(
			     "uioctl", fp, FILE_PRIVATE(fp)->meta_rf,
			     FILE_PRIVATE(fp)->meta_rf_len, filerq, freq_sz,
			     frep_sz, &pna, &node)) < 0) {
			goto bcast_done;
		}
		...
		for (nord = 0; nord < nnodes; nord++) {
			...
			if (rval < 0) {
				KDEBUG_OFC(
					0,
					"DVS: do_remote_ioctl: got error from "
					"server %d\n",
					rval);
				if (id->arg_rw) {
	...
	bcast_done:
		...
		elapsed_jiffies = jiffies;
		rval = send_ipc_file_retry("uioctl", fp,
					   FILE_PRIVATE(fp)->meta_rf,
					   FILE_PRIVATE(fp)->meta_rf_len, 0,
					   filerq, freq_sz, filerp, frep_sz,
					   &node);
		if (rval < 0)
			goto out;
		log_request(filerq->request, NULL, ip, fp, 1, node,
		...
		if (filerp->rval < 0) {
			KDEBUG_OFC(0,
				   "DVS: do_remote_ioctl: got error %ld from "
				   "server\n",
				   filerp->rval);
		}
		rval = filerp->rval;
...
/*
 * Set the new stripe configuration via the KDWFS_SET_STRIPE_CONFIG ioctl and
 * then update DVS stripe configuration for the open file.
 *
 * Note that we do not change the DVS block size or stripe width for non
 * regular files.  In the case of a directory, the parameters are set on the
 * DWFS directory for stripe configuration inheritance but DVS does not use
 * them.
 */
#ifdef WITH_DATAWARP
static long dvs_dwfs_set_stripe_config(struct file *fp, unsigned long arg)
{
	int rval;
	struct ioctl_desc id;
	struct ioctl_desc *idp;
	struct dvs_ioctl_tunnel *itun;
	struct dvs_dwfs_stripe_config dvs_conf;
	struct kdwfs_set_stripe_config *dwfs_conf;
	struct inode *ip = file_inode(fp);

	/* DW-1557: disable client side caching and custom striping */
	if (!FILE_DWFS(fp) || FILE_PRIVATE(fp)->cache)
		return -EINVAL;

	if (copy_from_user(&dvs_conf, (void __user *)arg,
			   sizeof(struct dvs_dwfs_stripe_config)))
		return -EFAULT;

	idp = get_ioctl_desc(DVS_TUNNEL_IOCTL);
	if (!idp)
		return -ENOSYS;
	memcpy(&id, idp, sizeof(struct ioctl_desc));

	id.arg_size = sizeof(struct dvs_ioctl_tunnel) +
		      sizeof(struct kdwfs_set_stripe_config);

	itun = kmalloc_ssi(id.arg_size, GFP_KERNEL);
	if (!itun)
		return -ENOMEM;

	itun->ioctl_cmd = KDWFS_SET_STRIPE_CONFIG;
	itun->arg_size = sizeof(struct kdwfs_set_stripe_config);
	itun->arg_by_ref = 1;

	dwfs_conf = (struct kdwfs_set_stripe_config *)itun->arg;
	dwfs_conf->stripe_width = dvs_conf.stripe_width;
	dwfs_conf->stripe_size = dvs_conf.stripe_size;

	inode_lock(ip);
	if (dwfs_conf->stripe_width > FILE_PRIVATE(fp)->data_rf_len &&
	    S_ISREG(ip->i_mode)) {
		inode_unlock(ip);
		printk(KERN_ERR "DVS: dwfs file %p invalid stripe width\n", fp);
		kfree_ssi(itun);
	...
	if (rval > 0 && S_ISREG(ip->i_mode)) {
		FILE_PRIVATE(fp)->data_rf_len = dwfs_conf->stripe_width;
		FILE_PRIVATE(fp)->blocksize = dwfs_conf->stripe_size;
	}
	inode_unlock(ip);
	kfree_ssi(itun);
	return rval;
...
static long uioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	case DVS_GET_REMOTE_FS_MAGIC:
		return put_user(INODE_PRIVATE(ip)->underlying_magic,
	...
	case DVS_GET_FILE_STRIPE_WIDTH:
		return put_user(FILE_PRIVATE(fp)->data_rf_len,
				(int __user *)arg);
		break;
	...
	case DVS_GET_FILE_DATASYNC:
		return put_user(FILE_PRIVATE(fp)->datasync,
				(short __user *)arg);
		break;
	...
	case DVS_SET_FILE_CACHE:
		...
		if (val == 0) {
			...
			if (INODE_CWC_FILES(ip)) {
				DVS_LOGP(
					"Warning: Caching disabled for writable "
			 ...
			 * caching anymore */
			if ((rval = filemap_write_and_wait(ip->i_mapping)) <
			    0) {
	...
	case DVS_GET_FILE_CLOSESYNC:
		return put_user(FILE_PRIVATE(fp)->closesync,
				(short __user *)arg);
		break;
	...
	case DVS_GET_FILE_KILLPROCESS:
		return put_user(FILE_PRIVATE(fp)->killprocess,
				(short __user *)arg);
		break;
	...
	case DVS_GET_FILE_DEFEROPENS:
		return put_user(FILE_PRIVATE(fp)->deferopens,
				(short __user *)arg);
		break;
	...
	case DVS_GET_NNODES:
		 ...
		 */
		return put_user(INODE_ICSB(ip)->data_servers_len,
				(int __user *)arg);
		break;
	...
	case DVS_BCAST_IOCTL:
		...
		if (itun->arg_size > 0) {
			if (sizeof(struct dvs_ioctl_tunnel) + itun->arg_size >
			    MAX_FILE_PAYLOAD) {
				printk(KERN_ERR "DVS: uioctl: arg too large, "
						"cmd 0x%x size %d\n",
				       cmd, itun->arg_size);
		...
		rval = do_remote_ioctl(fp, &id, (void *)itun);
		if (rval == 0) {
			if (copy_to_user((void __user *)arg, itun,
					 id.arg_size)) {
				rval = -EFAULT;
				goto out;
			}
	...
	case DVS_AUGMENTED_BCAST_IOCTL:
		atun = kmalloc_ssi(sizeof(struct dvs_augmented_ioctl_tunnel),
				   GFP_KERNEL);
		if (!atun) {
			rval = -ENOMEM;
			goto out;
		...
		id.arg_size = sizeof(struct dvs_augmented_ioctl_tunnel) +
			      atun->arg_size;
		...
		if (rval == 0) {
			if (copy_to_user((void __user *)arg, atun,
					 id.arg_size)) {
				rval = -EFAULT;
				goto out;
			}
		...
		break;
#ifdef WITH_DATAWARP
	case DVS_DWFS_SET_STRIPE_CONFIG:
		return dvs_dwfs_set_stripe_config(fp, arg);
		break;
#endif
	default:
		rval = -EINVAL;
				   ...
				   struct open_file_info *ofi)
{
	...
	/* enable FILE_CACHE for RO_CACHE so it can be disabled via CACHE env
	 * var */
	if (ofi->ro_cache) {
		ofi->cache = 1;
	}
	...
	if (env)
		rval = copy_from_user(env, (void *)mm->env_start, env_len);
	else
		rval = -ENOMEM;
	...
	*(env + env_len - 1) = '\0';
	...
	while ((i < env_len) && (*penv != '\0')) {
		...
		for (opt = 0; opt < numoptions; opt++) {
			...
			if (strncmp(penv, optionlist[opt].opt_name, optlen) !=
			    0) {
				continue;
			}
			...
			case DVS_DATASYNC_TYPE:
				if (strcmp(sval, "on") == 0)
					...
					ofi->datasync = 0;
				KDEBUG_OFC(0,
					   "DVS: %s: setting datasync to %d "
					   "for file %s\n",
					   __FUNCTION__, ofi->datasync, path);
				break;
			case DVS_CACHE_TYPE:
				if (strcmp(sval, "on") == 0) {
					ofi->cache = 1;
				...
				KDEBUG_OFC(0,
					   "DVS: %s: setting cache to %d for "
					   "file %s\n",
					   __FUNCTION__, ofi->cache, path);
				break;
			case DVS_BLOCKSIZE_TYPE:
				val = simple_strtol(sval, NULL, 0);
				if (val > 0) {
					KDEBUG_OFC(0,
						   "DVS: %s: setting blocksize "
						   "to %d for file %s\n",
			...
			case DVS_CLOSESYNC_TYPE:
				if (strcmp(sval, "on") == 0)
					...
					ofi->closesync = 0;
				KDEBUG_OFC(0,
					   "DVS: %s: setting sync on close to "
					   "%d for file %s\n",
					   __FUNCTION__, ofi->closesync, path);
				break;
			case DVS_KILLPROCESS_TYPE:
				if (strcmp(sval, "on") == 0)
					...
					ofi->killprocess = 0;
				KDEBUG_OFC(0,
					   "DVS: %s: setting killprocess "
					   "to %d for file %s\n",
					   __FUNCTION__, ofi->killprocess,
					   path);
				break;
			...
			case DVS_DEFEROPENS_TYPE:
				if (strcmp(sval, "on") == 0)
					...
					ofi->deferopens = 0;
				KDEBUG_OFC(0,
					   "DVS: %s: setting defer opens to "
					   "%d for file %s\n",
					   __FUNCTION__, ofi->deferopens, path);
				break;
			case DVS_CACHE_RD_SZ_TYPE:
				val = simple_strtol(sval, NULL, 0);
				if (val > 0 &&
				    (icsb->superblock->s_flags & MS_RDONLY)) {
					KDEBUG_OFC(
						0,
						"DVS: %s: setting cache_read_sz"
						" to %d for file %s\n",
						__FUNCTION__, val, path);
	...
	KDEBUG_OFC(0,
		   "DVS: %s: pid %d (%s): path %s: datasync %d closesync %d "
		   "cache %d blocksize %d nnodes %d killprocess %d "
		   "deferopens %d cache_read_sz %d\n",
		   __FUNCTION__, current->pid, current->comm, path,
		   ofi->datasync, ofi->closesync, ofi->cache, ofi->blocksize,
		   ofi->data_rf_len, ofi->killprocess, ofi->deferopens,
		   ofi->cache_read_sz);
}

static void remote_file_init(struct remote_file *rf, struct remote_file *source,
 ...
 */
static struct open_file_info *
alloc_init_file_private(char *path, struct file *fp,
			struct incore_upfs_super_block *icsb,
			int data_stripe_width, int meta_stripe_width)
{
	int i, rf_len;
...
static int uopen(struct inode *ip, struct file *fp, int retrying)
{
	...
	if (send_request) {
		...
		filerq->flags.multiple_servers =
			(INODE_ICSB(ip)->data_servers_len > 1 ||
			 INODE_ICSB(ip)->meta_servers_len > 1);
		decompose_open_flags(&filerq->u.openrq.flags, fp->f_flags);
		 ...
		 * write-caching
		 */
		if ((FILE_DWFS(fp)) || (INODE_ICSB(ip)->cache &&
					!(SUPER_SFLAGS(ip) & MS_RDONLY))) {
			filerq->u.openrq.flags.o_append = 0;
		}

		if (FILE_DWCFS(fp)) {
			/* truncate is handled by VFS, doesn't need to be
			 * forwarded, O_EXCL done by create */
			filerq->u.openrq.flags.o_excl = 0;
			filerq->u.openrq.flags.o_trunc = 0;
			filerq->flags.is_dwcfs_stripe =
				(INODE_ICSB(ip)->data_stripe_width > 1 &&
				 !INODE_ICSB(ip)->loadbalance);
		}
	}
	...
	if (!private_data_allocated) {
		...
		/* Regular non-datawarp, non-cache files get striping disabled
		   for O_APPEND */
		...
		if (IS_ERR(fp->private_data = alloc_init_file_private(
				   path, fp, icsb, data_stripe_width,
				   meta_stripe_width))) {
		...
		KDEBUG_QSC(0,
			   "Opening file %p path %s, possibly because of "
			   "quiescing\n",
			   fp, path);
		/*
		...
		if (FILE_PRIVATE(fp)->open_flags & O_CREAT) {
			KDEBUG_QSC(
				0,
				"uopen file %p path %s tagged with O_CREAT\n",
				fp, path);
			filerq->u.openrq.flags.o_creat = 1;
		}
	}
	...
	if (!send_request) {
		...
		if (FILE_DWFS(fp) && S_ISREG(file_inode(fp)->i_mode)) {
			KDEBUG_OFC(0,
				   "Calling create_dwfs_data_paths file %p "
				   "data_rf %p data_rf_len %d "
				   "meta_rf %p meta_rf_len %d\n",
				   fp, FILE_PRIVATE(fp)->data_rf,
				   FILE_PRIVATE(fp)->data_rf_len,
				   FILE_PRIVATE(fp)->meta_rf,
				   FILE_PRIVATE(fp)->meta_rf_len);
			if ((rval = create_dwfs_data_paths(
				     fp, &openrp->dwfs_info)) < 0) {
	...
	if (!FILE_PRIVATE(fp)->cache && INODE_CWC_FILES(ip) &&
	    S_ISREG(ip->i_mode) && !(SUPER_SFLAGS(ip) & MS_RDONLY)) {
		printk(KERN_ERR
		       "DVS: %s: Inode is currently open "
	...
	elapsed_jiffies = jiffies;
	if ((rval = send_multi_async_ipc_file_retry("uopen", fp, rf_array,
						    rf_len, filerq, rsz, rpsz,
						    &pna, &node_used)) < 0) {
		printk(KERN_ERR
		       "DVS: send_multi_async_ipc_file_retry failed with %d\n",
		       rval);
		goto uopen_error;
	}
	...
	KDEBUG_OFC(0,
		   "About to fill in remote_file structs: fp %p rf %p "
	...
	for (node = 0; node < rf_len; node++) {
		...
		if ((FILE_PRIVATE(fp)->ro_cache) && (node == 0)) {
			 ...
			 */
			if (FILE_PRIVATE(fp)->cache)
				FILE_PRIVATE(fp)->cache =
					frp->u.openrp.ro_cache_check;
			up(&FILE_PRIVATE(fp)->rocache_sema);
		}

		KDEBUG_OFC(0,
			   "DVS: uopen: got file handle 0x%p for "
			   "local file 0x%p size %Ld nord %d\n",
			   rf->file_handle.remote_ref, fp, frp->u.openrp.size,
			   node);
	}

	KDEBUG_OFC(0,
		   "DVS: uopen: file blocksize for fp 0x%p set to %d for inode "
		   "0x%p\n",
	 ...
	 */
	if (((ip->i_mode & S_IFMT) !=
		     (frp->u.openrp.inode_copy.i_mode & S_IFMT) ||
	     ip->i_ino != frp->u.openrp.inode_copy.i_ino) &&
	    !ignore_ino_mismatch(fp->f_path.dentry, ip)) {
		KDEBUG_OFC(0,
			   "DVS: %s: deleting dentry 0x%p for inode 0x%p "
			   "(%ld %ld %d %d), path %s\n",
			   __FUNCTION__, fp->f_path.dentry, ip, ip->i_ino,
			   frp->u.openrp.inode_copy.i_ino, ip->i_mode,
			   frp->u.openrp.inode_copy.i_mode, path);
		delete_dentry = 1;
		goto uopen_error;
	}
	...
	KDEBUG_OFC(
		0,
		"DVS: %s: updating inode 0x%p (%ld), path %s new size %lld\n",
	 ...
	 */
	if (!FILE_PRIVATE(fp)->rip && FILE_PRIVATE(fp)->cache &&
	    !(SUPER_SFLAGS(ip) & MS_RDONLY)) {
		inval = 1;
		INODE_INC_CWC_FILES(ip);
	} else {
	...
	update_inode(&frp->u.openrp.inode_copy, ip, fp->f_path.dentry, fp,
		     inval);
...
uopen_error:
	...
	if (urelease_common(ip, fp, retrying)) {
...
static int deferred_uopen(struct inode *ip, struct file *fp, int nord)
{
	...
	filerq->flags.is_dwcfs_stripe =
		(FILE_DWCFS(fp) && INODE_ICSB(ip)->data_stripe_width > 1 &&
		 !INODE_ICSB(ip)->loadbalance);
	...
	if (FILE_DWFS(fp)) {
		filerq->u.openrq.flags.o_rdonly = 0;
		filerq->u.openrq.flags.o_wronly = 0;
		filerq->u.openrq.flags.o_rdwr = 1;
	}

	KDEBUG_OFC(0,
		   "DVS: deferred_uopen: called ip 0x%p path %s flags 0x%x\n",
		   ip, path, fp->f_flags);
	...
	elapsed_jiffies = jiffies;
	rval = send_ipc_file_retry("deferred_uopen", fp,
				   FILE_PRIVATE(fp)->data_rf,
				   FILE_PRIVATE(fp)->data_rf_len, nord, filerq,
				   rsz, filerp, sizeof(struct file_reply),
				   &node);
	if (rval < 0) {
		goto deferred_uopen_done;
	}
	...
	if (filerp->rval < 0) {
		KDEBUG_OFC(0,
			   "DVS: deferred_uopen: got error from server %ld\n",
			   filerp->rval);
		rval = filerp->rval;
		goto deferred_uopen_done;
	} else {
	...
	KDEBUG_OFC(
		0,
		"DVS: deferred_uopen: got file handle 0x%p for local file 0x%p "
		" nord %d path %s\n",
	...
	if (FILE_DWFS(fp) || FILE_DWCFS(fp)) {
		KDEBUG_OFC(0,
			   "DVS: deferred_uopen: deferred open for DWFS data "
		...
		rval = -ENOENT;
		printk(KERN_INFO
		       "DVS: %s: open() returned %d for %s (pid "
...
static int uflush(struct file *fp, fl_owner_t id)
{
	...
	elapsed_jiffies = jiffies;
	if ((rval = send_multi_async_ipc_file_retry(
		     "uflush", fp, FILE_PRIVATE(fp)->data_rf,
		     FILE_PRIVATE(fp)->data_rf_len, filerq,
		     sizeof(struct file_request), sizeof(struct file_reply),
		     &pna, &node)) < 0) {
		goto uflush_done;
	}
	log_request(filerq->request, NULL, file_inode(fp), fp, nnodes, node,
		    jiffies - elapsed_jiffies);
...
static int urelease_common(struct inode *ip, struct file *fp, int retry)
{
	...
	if (!retry && FILE_PRIVATE(fp)->cache &&
	    !((SUPER_SFLAGS(ip) & MS_RDONLY))) {
		...
		if (rval < 0)
			...
			KDEBUG_OFC(
				0,
				"DVS: %s: completed filemap_write_and_wait\n",
				__FUNCTION__);
...
retry_requests:
	...
	while (!retry && !list_empty(&iip->requests)) {
		...
		if ((dcount != 1 /*detach*/) && rval) {
			/*
			 * We detached this file from the pending requests so
			 * let this close happen.  We know the detach is only
			 * successful when the server is no longer using our
			 * file pointer for any outstanding operations.
			 */
			break;
		}
	}
	...
	while ((retry == 0) && iip->oio) {
		...
		if (ct == 0) {
			KDEBUG_OFC(
				0,
				"DVS: urelease: waiting for page cache request\n");
			ct++;
		}
		cond_resched();
		if (!IGNORE_INODE_SEMAPHORE(iip)) {
	...
	filerq->u.closerq.sync =
		FILE_PRIVATE(fp)->closesync && !FILE_PRIVATE(fp)->datasync;
	filerq->retry = INODE_ICSB(ip)->retry;
	...
	if (FILE_PRIVATE(fp)->ro_cache) {
		...
		} else if (((FILE_PRIVATE(fp)->open_flags & O_ACCMODE) ==
			    O_RDWR) ||
			   ((FILE_PRIVATE(fp)->open_flags & O_ACCMODE) ==
			    O_WRONLY)) {
	...
	elapsed_jiffies = jiffies;
	if ((rval = send_multi_async_ipc_file_retry(
		     "urelease", fp, FILE_PRIVATE(fp)->rf,
		     FILE_PRIVATE(fp)->rf_len, filerq,
		     sizeof(struct file_request), sizeof(struct file_reply),
		     &pna, &node)) < 0) {
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		if (rval < 0) {
			KDEBUG_OFC(
				0,
				"DVS: urelease: got error %ld from nord %d\n",
				frp->rval, nord);
			break;
		}
	}
...
urelease_nearly_done:
	...
	for (nord = 0; nord < nnodes; nord++) {
		...
		if (node_map[rf->remote_node].server_info &&
		    rf->finfo == ((struct open_file_info *)fp->private_data) &&
		    !list_empty(&DATA_RF(fp, nord)->list) && rf->valid) {
			server_info = node_map[rf->remote_node].server_info;
			spin_lock(&server_info->lock);
			list_del_init(&rf->list);
			spin_unlock(&server_info->lock);
			DVS_BUG_ON(atomic_dec_return(&server_info->open_files) <
				   0);
		}
	}
	if (!FILE_PRIVATE(fp)->rip) {
...
static int ufsync(struct file *fp, loff_t off1, loff_t off2, int datasync)
{
	...
	if (FILE_ICSB(fp)->multi_fsync) {
		...
		nnodes = FILE_PRIVATE(fp)->data_rf_len;
		if ((rval = send_multi_async_ipc_file_retry(
			     "ufsync", fp, FILE_PRIVATE(fp)->data_rf,
			     FILE_PRIVATE(fp)->data_rf_len, filerq,
			     sizeof(struct file_request),
			     sizeof(struct file_reply), &pna, &node)) >= 0) {
		...
		elapsed_jiffies = jiffies;
		rval = send_ipc_file_retry("ufsync", fp,
					   FILE_PRIVATE(fp)->meta_rf,
					   FILE_PRIVATE(fp)->meta_rf_len, 0,
					   filerq, sizeof(struct file_request),
...
static int ufasync(int arg1, struct file *fp, int arg3)
{
	...
	elapsed_jiffies = jiffies;
	rval = send_ipc_file_retry("ufasync", fp, FILE_PRIVATE(fp)->meta_rf,
				   FILE_PRIVATE(fp)->meta_rf_len, 0, filerq,
				   sizeof(struct file_request), filerp,
				   sizeof(struct file_reply), &node);
	if (rval >= 0)
...
static int ulock(struct file *fp, int cmd, struct file_lock *fl)
{
	...
	if ((cmd != F_GETLK) && (lk.l_type != F_UNLCK)) {
		...
		if (rval != 0) {
			KDEBUG_OFC(0,
				   "DVS: ulock: client posix_lock_file_wait"
	...
	do {
		elapsed_jiffies = jiffies;
		rval = send_ipc_file_retry("ulock", fp,
					   FILE_PRIVATE(fp)->meta_rf,
					   FILE_PRIVATE(fp)->meta_rf_len, 0,
					   filerq, sizeof(struct file_request),
					   filerp, sizeof(struct file_reply),
		...
		if (rval < 0) {
			...
			if (fl->fl_type == F_UNLCK) {
				if (rval != -ERESTARTSYS) {
					(void)posix_lock_file_wait_retry(fp,
									 fl);
				}
			}
			/*
			...
			if (cmd == F_GETLK) {
				...
				/*
				 * The lock is created/removed on the client
				 * node as well as on the the server node
				 * because the kernel does not call the vfs
				 * layer to remove posix locks at file close
				 * unless f_path.dentry->d_inode->i_flock is
				 * set.  We have to use real locks instead of
				 * just plugging in a temporary non-NULL value
				 * into i_flock because multiple files can be
				 * referencing the same inode, and there's no
				 * good way to make that safe. F_[RD|WR]LCK are
				 * handled above.  Do client unlock if
				 * everything else was successful.
				 */
				if (fl->fl_type == F_UNLCK) {
					rval = posix_lock_file_wait_retry(fp,
									  fl);
					if (rval != 0 &&
...
static ssize_t ureadv(struct file *fp, const struct iovec *iovp,
...
/* NOTE: readv/writev NOT CURRENTLY USED */
static ssize_t uwritev(struct file *fp, const struct iovec *iovp,
		       unsigned long count, loff_t *off)
{
	return ((ssize_t)-ENOTSUPP);
}
...
static int uwritepage(struct page *pagep, struct writeback_control *wbc)
{
	...
	if (pagep->index == (size >> PAGE_SHIFT))
		len = size & ~PAGE_MASK;
	else
		len = PAGE_SIZE;
			   ...
			   struct writeback_control *wbc)
{
	...
	size = i_size_read(ip);
	if ((index + ep->count - 1) == (size >> PAGE_SHIFT))
		len = size & ~PAGE_MASK;
	else
		len = PAGE_SIZE;

	/* calculate total length:  initial full pages + final page */
	len += (ep->count - 1) * PAGE_SIZE;

	KDEBUG_OFC(0,
		   "DVS: %s: ep indx %d ep count %d pagesrq 0x%p index %lld len"
		   " %lu pagesd 0x%p ep 0x%p ip 0x%p size %lld wbc 0x%p\n",
		   __FUNCTION__, ep->indx, ep->count, pagesrq, index, len,
		   pagesd, ep, ip, size, wbc);
	...
	if (len == 0) {
		printk(KERN_ERR "DVS: %s: zero length write_extent request. "
		       ...
		       __FUNCTION__, ip, wbc, index, ep->count);
		rval = -EFAULT;
	...
	pagesrq->offset = index * PAGE_SIZE; /* real offset into file */
	pagesrq->length = len;
...
error:
	...
	for (i = 0; i < ep->count; i++) {
		...
		put_page(pagesd->pages[ep->indx + i]);
	}
 ...
 */
static int uwritepages(struct address_space *mapping,
		       struct writeback_control *wbc)
{
	int rval = 0;
	int done = 0;
	...
	KDEBUG_OFC(
		0,
		"DVS: %s: called. map 0x%p nr_to_write %ld start %lld end %lld"
	...
	if (npages > wb_threshold_pages)
		npages = wb_threshold_pages; /* 8Mb worth of pages to match the
						wb size */
...
retry:
	...
	while (!done && (index <= end)) {
		...
		KDEBUG_RPS(
			0,
			"DVS: %s: map 0x%p wbc 0x%p ext 0x%p ep 0x%p index %lu <= "
			"end %lu done %lu\n",
			__FUNCTION__, mapping, wbc, extents, ep, index, end,
			done_index);

		nr_pages = find_get_pages_tag(mapping, &index, tag, npages,
					      pagesd->pages);
		if (nr_pages == 0)
			break;
		...
		if (ep->count != 0) {
			...
			if (rval < 0) {
				printk(KERN_ERR
				       "DVS: %s: last extent write "
				       "failed. rval %d ip 0x%p ep 0x%p wbc 0x%p "
				       "map 0x%p i %d\n",
				       __FUNCTION__, rval, ip, ep, wbc, mapping,
				       i);
				done = 1;
			}
		}
...
ureadpage_old(struct file *fp, struct page *pagep)
{
	...
	offset = (loff_t)pagep->index << PAGE_SHIFT;
...
#define PREFETCH ((MAX_MSG_SIZE - sizeof(struct file_request)) & PAGE_MASK)
	...
	found = NULL;
	while (iop) {
		if ((offset >= iop->offset) &&
		    (offset < (iop->offset + iop->length))) {
			found = iop;
			break;
		}
		iop = iop->next;
	...
	rval = send_ipc_request_async_stats(INODE_ICSB(ip)->stats, node,
					    RQ_FILE, filerq,
					    sizeof(struct file_request), NULL,
					    0, DATA_RF(fp, nord)->identity);
	if (rval == -EHOSTDOWN) {
		down(&aretrysem[node]);
		list_for_each_safe (lp, tp, &alist[node]) {
			p = list_entry(lp, struct async_retry, list);
			if (p == aretry) {
				/*
				 * If async_retry is not AR_Default it is
				 * already being handled by async_op_retry
				 */
				if (p->ar_status != AR_Default)
					break;

				p->ar_status = AR_Retrying;
				up(&aretrysem[node]);
				rval = send_ipc_file_retry(
					"ureadpage", fp,
					FILE_PRIVATE(fp)->data_rf,
					FILE_PRIVATE(fp)->data_rf_len, nord,
					filerq, sizeof(struct file_request),
					NULL, 0, &node_used);
				...
				/*
				 * If FILE_NODE() has changed,
				 * send_ipc_file_retry/ file_ops_retry has
				 * resent this request to a new server. Adjust
				 * which alist is used for replies. Note that if
				 * a reply arrives before the lists are
				 * corrected below the reply could be missed,
				...
				if (aretry->ar_node !=
					...
					up(&aretrysem[node]);
					down(&aretrysem[DATA_RF(fp, nord)
								->remote_node]);
					aretry->ar_node = node =
						DATA_RF(fp, nord)->remote_node;
...
static int ureadpage(struct file *fp, struct page *pagep)
{
	...
	KDEBUG_RPS(0,
		   "DVS: %s: cpu %d readpage for fp 0x%p (ip 0x%p) index %ld\n",
		   __FUNCTION__, raw_smp_processor_id(), fp, file_inode(fp),
		   pagep->index);
...
static void dvs_read_complete(void *req, int status, void *addr, size_t length)
{
	...
	KDEBUG_RPS(
		0,
		"DVS: %s: cpu %d rq 0x%p stat %d addr 0x%p len %ld remain %d\n",
		__FUNCTION__, raw_smp_processor_id(), rq, status, addr, length,
		reads_remaining);
	...
	if (!reads_remaining) {
		...
		if (status) {
			 ...
			 * rq_msg_waiters (detach_file_from_reqs() today)
...
static void adjust_filerq(struct usiipc *msg, int to_node)
{
	...
	/*
	 * Since we're going to likely mess with the request, lock the (old)
	 * aretry chain to claim ownership temporarily.
	 */
	down(&aretrysem[orig_node]);

	DVS_BUG_ON(freq->request != RQ_READPAGES_RQ &&
	...
	for (i = 0; i < rq->msg_count; i++) {
		...
		if ((fip = &rq->freqs[i])->freq) {
			if (fip->freq == freq) {
				DVS_BUG_ON(fip->to_node != orig_node);
				fip->to_node = to_node;
				aretry = fip->aretry;
			}
			actual_msgs++;
	...
	KDEBUG_RPS(
		0,
		"DVS: %s: adjust freq 0x%p from node %d to %d reads %d msgs %d\n",
	...
	if ((msgs = atomic_read(&rq->msgs_outstanding)) != actual_msgs) {
		KDEBUG_RPS(0, "DVS: %s: cpu %d adjust msgs out from %d to %d\n",
			   __FUNCTION__, raw_smp_processor_id(), msgs,
			   actual_msgs);
		atomic_set(&rq->msgs_outstanding, actual_msgs);
	}
	if ((reads = atomic_read(&rq->reads_outstanding)) != actual_reads) {
		KDEBUG_RPS(0,
			   "DVS: %s: cpu %d adjust reads out from %d to %d\n",
			   __FUNCTION__, raw_smp_processor_id(), reads,
			   actual_reads);
	...
	if (switch_chains) {
		DVS_BUG_ON(!aretry || !async_op_valid(aretry, orig_node) ||
			   aretry->ar_status != AR_Retrying);
...
#define INVALID -1
	...
	enum { re_unset, re_mapped, re_send_loop } re_state = re_unset;

	/*
	 * Use the blocksize listed on the mount line for the DWFS case since
	 * that's the only place the data lives. Also, since we're using
	 * get_length_for_offset() to calculate the transfer lengths, we need to
	 * calculate num_blks based on where our offset is in the first block.
	 * For example, a 16k read with an 8k block size could consist of either
	 * two 8k reads or one 4k read, one 8k read, and one 4k read depending
	 * on what the offset is.
	 */
	if (FILE_DWFS(fp)) {
	...
	/*
	 * OK, we've now got the number of blocks and their size.  We know that
	 * the entire I/O will fit inside the blocks but may be a subset.  The
	 * blocks will be distributed to one or more servers based on the
	 * distribution algorithm (DWFS or normal page-cache behavior)
	 *
	 ...
	 *
	 * Larger blocksizes will simply be multiple IOVs (transfers) to span
	 * the block.  If the block size is larger than the maximum transfer
	 * size, there may be extra transfers in the block not required for the
	 * I/O. We'll calculate the number of transfers needed here.
	 */
	...
	if (rps_blksize > max_transport_msg_size) {
		...
		num_xfers +=
			(num_blks > 1) ?
				((((last_blk_res = ((rq->offset + rq->length) %
						    rps_blksize)) ?
					   last_blk_res :
					   rps_blksize) +
				  max_transport_msg_size - 1) /
		...
		/*
		 * Now for every block in the middle.  These are easy since we
		 * know they're all full blocks.
		 */
		num_xfers += (num_blks > 2) ? (num_blks - 2) * xfers_blk : 0;
	} else {
		num_xfers = num_blks;
	}
	...
	rq->vmap_addr = xfer_addr =
		vmap(&rq->pagesd->pages[rq->ext_indx], rq->ext_count, VM_MAP,
		     PAGE_KERNEL_EXEC);
	if (!rq->vmap_addr) {
	...
	KDEBUG_RPS(0,
		   "DVS: %s: cpu %d mapped request 0x%p to 0x%p now reading\n",
		   __FUNCTION__, raw_smp_processor_id(), rq, rq->vmap_addr);

	rma_handle = map_ipc_kernel_memory(rq->vmap_addr, rq->length,
					   DVS_NEED_HANDLER);
	...
	DVS_BUG_ON(count_ipc_memory(rma_handle)); /* old page pointers scheme
						     not used by LNet, consider
						     removing */
	...
	while (xfer_len) {
		...
		/* For DWFS, we need to make sure that we only read within our
		 * stripe. */
		if (FILE_DWFS(fp)) {
			this_xfer = get_length_for_offset(
				fp, xfer_len, xfer_off, rps_blksize);
		} else {
			this_xfer = (xfer_len >= rps_blksize) ? rps_blksize :
								xfer_len;
		}
		...
		while (this_xfer_remaining) {
			this_xfer =
				(this_xfer_remaining > max_transport_msg_size) ?
					max_transport_msg_size :
					this_xfer_remaining;

			/*
			 * Get the compressed index of this nord so we don't
			 * always have to examine each one.  That way we can
			 * roll through these in one pass.
			 */
	...
	if (unlikely(iovs_used != num_xfers)) {
		printk(KERN_EMERG
		       "DVS: IOV mismatch! allocated %d used %d blksize %ld"
		       " offset %Ld length %ld DW %d servers %d\n",
		       num_xfers, iovs_used, rps_blksize, rq->offset,
		       rq->length, FILE_DWFS(fp), nnodes);
		DVS_BUG();
	}
	...
	/*
	 * The number of reads is 1-1 with the number of IOVs.  These may
	 * originate from the same server or from multiple servers.  Initialize
	 * this to the maximum value and it will be adjusted if there were
	 * errors.
	 */
	atomic_set(&rq->reads_outstanding, iovs_used);

	/*
	 * Set msgs_outstanding to its maximum value here to keep it around
	 * until we're done sending all the messages (can't increment it upward
	 * from zero since a message could come back and free it while we were
	 * trying to send the next one)  We'll come back and decrement it if
	 * there are errors and messages weren't able to be sent.
	 *
	...
	for (nord_indx = 0; nord_indx < nodes_used; nord_indx++) {
		 ...
		 * Will be the same or less than the number of reads.  For
		 * example:
		 *
		...
		ipr->rma_handle = rma_handle; /* server needs to know if RMA vs.
						 piggyb */

		/*
		 * The I/O on the server is driven off the IOVs, the base and
		 * length here are just an overview of the whole I/O and are
		 * technically not correct when there are multiple servers.
		 */
		...
		/*
		 * Transfer the IOVs to the message payload.  When the
		 * message(s) come back, each request (file_request) will
		 * traverse the IOVs to map it(them) back to the pages they're
		 * supposed to clean up and release.
		 *
		 * While we do have more convenient mechanisms on the client
		 * which directly identify the pages in the request, it's just
		 * easier to parse the IOVs since the request may have come back
		 * short so we need to base our actions on the server response.
		 */
		j = ni->iov_list;
		for (i = 0; i < ni->iov_count; i++) {
			...
			KDEBUG_RPS(
				0,
				"DVS: %s: cpu %d inode 0x%p filerq 0x%p nord %d "
		...
		KDEBUG_RPS(
			0,
			"DVS: %s: cpu %d sending node %d %d IOVs for inode 0x%p "
			"filerq 0x%p request 0x%p\n",
			__FUNCTION__, raw_smp_processor_id(), ni->nord,
			ni->iov_count, rq->ip, filerq, rq);

		DVS_TRACEL("procext+", ni->nord, ni->iov_count, rq->ip, filerq,
			   rq);
		...
		aretry->ar_status = AR_Default;
		aretry->ar_retry_count = 0;

		KDEBUG_RPS(
			0,
			"DVS: %s: cpu %d aretry 0x%p for inode 0x%p filerq 0x%p\n",
			__FUNCTION__, raw_smp_processor_id(), aretry, rq->ip,
			filerq);
		...
		/* Check the dynamic number of freqs created by
		 * init_pages_request */
		if (++rq->msg_count > rq->num_freqs) {
			printk(KERN_ERR
			       "DVS: %s: Overflowing pages_request num_freqs.\n",
			       __FUNCTION__);
			rval = -ENOBUFS;
			goto error;
		}
		...
		/*
		 * Add this file request to the pages_request_t so we can locate
		 * all the file requests if we need to cancel the transaction.
		 * Also keep track of the number of reads in the file request
		 * instance in case we need to adjust the counters/etc for
		 * failover or errors.
		 */
		...
		/*
		 * Tell the IPC layer to let us know if something went wrong
		 * with the send to_node like retry, failover, etc.
		 */
		NOTIFY_OF_ABNORMAL_SEND(filerq, to_node);

		elapsed_jiffies = jiffies;
		rval = send_ipc_request_async_stats(
			INODE_ICSB(rq->ip)->stats, to_node, RQ_FILE, filerq,
			request_size, NULL, 0, DATA_RF(fp, ni->nord)->identity);
		if (rval < 0) {
			KDEBUG_RPS(
				0,
				"DVS: %s: cpu %d xmit error inode 0x%p filerq 0x%p nord %d "
				"iov %d\n",
				__FUNCTION__, raw_smp_processor_id(), rq->ip,
				filerq, ni->nord, i);
			...
			if (rval == -EHOSTDOWN) {
				...
				list_for_each_safe (lp, tp, &alist[to_node]) {
					thisa = list_entry(
						lp, struct async_retry, list);
					if (thisa == aretry) {
						/*
						 * If async_retry is not in the
						 * default state, it's already
						 * being handled by
						 * async_op_retry.
						 */
						if (thisa->ar_status !=
	...
	/*
	 * These are the things that are free'd regardless of whether or not
	 * there were any errors.
	 */
	if (node_info_map) {
		kfree_ssi(node_info_map);
	}
	if (node_info) {
		kfree_ssi(node_info);
	}
...
error:
	/*
	 * Something obviously went wrong either initially or down the road
	 * after perhaps 1 or more messages had been sent.
	 *
	 ...
	 *     messages?  If so we'd need to get into the IOVs to see what's
	 * left.
	 *   - async retry and the file_request can be deleted since if we hit
	 * an error it was on the last (which could have been first) message.
	...
	if (re_state >= re_send_loop) {
		/*
		 * Start with the async retry and file request since we don't
	...
	if (!rq->rq_flags & RPS_RQ_FLAGS_READPAGE) {
		if (re_state < re_send_loop) {
			/*
			 * Never made it to the send loop, error the whole page
			 * range since it wasn't ever touched.
			 */
			atomic_set(&rq->reads_outstanding, 0);
			error_page_range(&rq->pagesd->pages[rq->ext_indx],
					 rq->ext_count, 1,
					 rq->rq_flags & PIO_RQ_FLAGS_WRITEPAGE);
		} else {
			/*
			 * Handle erroring the pages here since we understand
			 * what's been sent out and what didn't make it.  Work
			 * directly off the built up IOVs to error and unlock
			 * anything that hadn't been processed.
			 */
			for (; nord_indx < nodes_used; nord_indx++) {
				...
				for (k = 0; k < ni->iov_count; k++) {
					reads = atomic_dec_return(
						&rq->reads_outstanding);
					DVS_BUG_ON(reads < 0);

					iov_to_rq_pages(&iovs[iov].usi_iov, rq,
							&start_page, &end_page);

					error_page_range(
						&rq->pagesd->pages[start_page],
						(end_page - start_page) + 1,
						1 /* release */,
						rq->rq_flags &
							PIO_RQ_FLAGS_WRITEPAGE);
					iov = iovs[iov].iov_next;
				}
			}

			/*
			 * Wake anybody who returned while we were erroring the
			 * pages.
			 */
			RELEASE_READ_WAITERS(rq);
		}
	 ...
	 * the rest of the stuff.  Easiest to divide it into "did we send
	 * anything" vs. not.
	 */
	if (re_state >= re_send_loop && msgs_successfully_sent) {
		/*
		 * Adjust the msgs_outstanding by the number that didn't make it
		 * but leave one at this point to hold the request since the
		 * ones that did make it would be looking to clean up when they
		 * returned.
		 */
		...
		if (atomic_dec_return(&rq->msgs_outstanding)) {
			/*
			 * One or more messages were sent successfully but at
			 * least one didn't make it and the intended recipient
			 * wasn't marked as down. We already errored the pages
			 * for the message(s) that didn't make it. Mark the
			 * request as invalid which will cause any outstanding
			 * file requests to be errored when they arrive -- just
			 * to be safe. Any messages that successfully arrived of
			 * course will have valid pages which we can't really
			 * change at this point.
			 *
			 * And yes, we're going to gamble that from the time it
			 * took us to decrement the msgs_outstanding to the time
			 * we set the state below that the request didn't
			 * disappear..
			 */
			atomic_set(&rq->state, RPS_RQ_INVALID);

			/* Wait for outstanding messages so we can clean up and
			 * error out rather than forcing uwritepages to handle
			 * cleanup */
			if (rq->rq_flags & PIO_RQ_FLAGS_WRITEPAGE) {
				down(&rq->writepages_sema);
			...
			/*
			 * Whatever we sent is now back so let's just treat this
			 * like a normal request that will disappear when the
			 * list is cleaned up.
			 */
			(void)finalize_request(rq);
		}
		...
		/*
		 * Failed early enough that nothing was done.  Clean up the
		 * request since we own it.
		 */
 ...
 *      win (others backoff) if an async entry is in the process of being
 * retried.
 */
		  ...
		  int to_node, unlink_mode_t unlnk_mode)
{
	 ...
	 * searching the async. retry list and if it's found we're good.  If
	 * not, somebody cancelled the request and cleaned up -- including
	 * free'ing freq.
	 */
	down(&aretrysem[to_node]);
	list_for_each_entry (aentry, &alist[to_node], list) {
		if ((aentry->filerq == freq) &&
		    (aentry->tx_cookie == tx_cookie)) {
			DVS_BUG_ON(freq->ipcmsg.seqno != tx_cookie);

			/*
			 * UNLNK_ESTALE_Retry is special - it finds the
			 * async_retry struct and sees if we should retry the
			 * request, and reflects that in the return value.
			 */
			if (unlnk_mode == UNLNK_ESTALE_Retry) {
				if (++freq->u.iopagesrq.estale_retries <=
				    estale_max_retry) {
			...
			/*
			 * This is our reply so obviously the retry was
			 * successful. No need to defer to this one now.
			 */
			if (aentry->ar_status == AR_Retrying &&
			...
			case AR_Retrying:
				/*
				 * We're not going to mess with the running
				 * failover code. Let's give them some time and
				 * try again.
				 */
				up(&aretrysem[to_node]);
				rval = -EAGAIN;
			...
			/*
			 * Mark this as expiring so that new requests coming in
			 * won't use it. This could be because we've already
			 * processed part of the request and it somehow got
			 * faulted out and the new request coming in would need
			 * to actually be a new request.
			 */
			rp_req = &freq->u.iopagesrq;
			rptr = rp_req->rq;
			rq_state = atomic_cmpxchg(&rptr->state, RPS_RQ_ACTIVE,
						  RPS_RQ_EXPIRING);
			break;
		}
	}
	...
	KDEBUG_RPS(0,
		   "DVS: %s: unlinked freq 0x%p aretry 0x%p node %d rq 0x%p\n",
		 ...
		 struct io_pages_reply *rp, processiovs_mode_t mode)
{
	...
	KDEBUG_OFC(
		0,
		"DVS: %s: entering process_iovs. rq 0x%p ipr 0x%p rp 0x%p mode %d\n",
	...
	if (mode != PRIOV_Cleanup) {
		if (mode == PRIOV_Error_Messenger) {
			...
			/*
			 * Do a quick sanity check on the error messenger to
			 * make sure everything is still valid.  We know that
			 * since there were errors, the transaction couldn't
			 * have completed in a normal fashion and this means
			 * everything should still be valid but we'll verify.
			 */
			...
			DVS_BUG_ON(io_req->state != RPS_RPSRQ_INUSE ||
				   rq_state == RPS_RQ_FREE ||
				   rq_state == RPS_RQ_EXPIRED);
		}

		/*
		 * Account for the RMAs that didn't happen due to either file
		 * system errors or an actual return value of 0 bytes which DVS
		 * IPC on the server would ignore and not do the LNetPut().
		 *
		 * Once we're through with these adjustments, check that we
		 * didn't just finish off all the reads as a result and if
		 * that's the case, wake any waiters.
		 *
		 * Note that writes perform RMAs prior to the file system call
		 * so an RMA could have taken place even if the write system
		 * call failed.
		 */
	...
	if ((reads = atomic_read(&rq->reads_outstanding))) {
		/*
		 * The message is back or cancelled but the reads are apparently
		 * lagging. Wait for them to complete since we'll be potentially
		 * zeroing/etc. Note that the read callback does pass up the
		 * offset and the length of each read as it completes but we
		 * don't know the overall return status to know if the original
		 * read request was short so best at this point just to wait for
		 * everything to finish before mucking with anything.
		 *
	...
	DVS_BUG_ON((rq->rq_flags & PIO_RQ_FLAGS_WRITEPAGE) && (!wbc || !iip));
	...
	for (i = 0; i < ipr->count; i++) {
		 ...
		 * this iov.
		 */
		iov_to_rq_pages(&ipr->iov[i], rq, &iov_start_page,
				&iov_end_page);

		KDEBUG_RPS(
			0,
			"DVS: %s: got rval %ld for request 0x%p iov %d iov_off %Ld "
			"iov_count %ld(pages %ld) flags %d wbc 0x%p\n",
			__FUNCTION__, this_rval, rq, i, ipr->iov[i].offset,
			ipr->iov[i].count, ipr->iov[i].count / PAGE_SIZE,
			rq->rq_flags, wbc);
		DVS_TRACEL("RPSprIOV", rq->ip, i, ipr->iov[i].offset,
			   ipr->iov[i].count, this_rval);
		DVS_TRACEL("prociov", rq, rq->ip, iov_start_page, iov_end_page,
			   wbc);

		KDEBUG_OFC(0, "DVS: %s: before page loop: sp %d ep %d\n",
			   __FUNCTION__, iov_start_page, iov_end_page);
		...
		for (j = iov_start_page; j <= iov_end_page; j++) {
			...
			if (rq->rq_flags & PIO_RQ_FLAGS_WRITEPAGE) {
				KDEBUG_RPS(
					0,
					"DVS: %s: wb reply. rq 0x%p j %d rval %ld ct %lu pg 0x%p "
					"wbc 0x%p\n",
					__FUNCTION__, rq, j, this_rval,
					ipr->iov[i].count, page, wbc);
				...
				/* clear writeback flag and unlock page now that
				 * IO has completed */
				end_page_writeback(page);
				unlock_page(page);
				put_page(page);
				...
				KDEBUG_OFC(
					0,
					"DVS: %s: rp reply. rq 0x%p j %d rval %ld ct %lu pg 0x%p\n",
					__FUNCTION__, rq, j, this_rval,
					ipr->iov[i].count, page);

				if (this_rval < 0) {
					if (!rval) {
						rval = this_rval; /* use first
								     error as
								     the return
								     value */
					}
					/*
					 * File system error for this iov, mark
					 * all its pages as invalid.  Remember
					 * that an iov may be a portion of an
					 * extent.
					 */
					KDEBUG_RPS(
						0,
						"DVS: %s: fs error %ld for inode 0x%p request 0x%p\n",
						__FUNCTION__, this_rval, rq->ip,
						rq);
					SetPageError(page);
				} else {
					loff_t page_offset =
					...
					loff_t io_last =
						ipr->iov[i].offset + this_rval;
					loff_t page_last =
						page_offset + PAGE_SIZE;

					if (io_last < page_last) {
						/*
						 * I/O fell short for the
						 * extent.  Check that this page
						 * is still part of the file
						 * first.
						 */
						if (page_offset >=
						    rq->ip->i_size) {
							KDEBUG_RPS(
								0,
								"DVS: %s: trunc'ed inode 0x%p on request 0x%p "
							...
							/* pre-reading a page
							 * for cache write, zero
							 * it */
							if (rq->rq_flags &
							    PIO_RQ_FLAGS_PREREAD) {
								datap = rq->vmap_addr +
									((j -
									  rq->ext_indx) *
									 PAGE_SIZE);
							...
							/*
							 * set a pointer into
							 * the actual page since
							 * it looks like we've
							 * got to clear part or
							 * all of the page.
							 */
							datap = rq->vmap_addr +
								((j -
							...
							KDEBUG_RPS(
								0,
								"DVS: %s: short read inode 0x%p on request 0x%p "
...
int finalize_request(pages_request_t *rq)
{
	...
	DVS_BUG_ON(atomic_read(&rq->msgs_outstanding));
	DVS_BUG_ON(atomic_read(&rq->reads_outstanding));
	DVS_BUG_ON(state == RPS_RQ_EXPIRED || state == RPS_RQ_ACTIVE ||
		   state == RPS_RQ_FREE);
	...
	if (!atomic_dec_return(&rq->pagesd->ref_count)) {
		KDEBUG_RPS(0,
			   "DVS: %s: free pages 0x%p pagesd 0x%p inode 0x%p\n",
	 ...
	 * expired or invalid.  If it's invalid, we processed part of it
	 * normally and others were errored.  We're done with it now so mark it
	 * normal so that it'll get cleaned up.
	 */
...
int detach_file_from_reqs(struct file *fp)
{
	 ...
	 * requests semaphore (and wanting the inode mutex which we hold) so
	 * we'll back off (as a closer) if there's an active writer to prevent
	 * deadlock.
	 */
			...
			if (rq->fp == fp) {
				if (another_fp || (rescan == rescanning)) {
					for (i = 0; i < rq->msg_count; i++) {
						...
						/*
						 * Grab a local copy since
						 * failover might be running and
						 * changing fip->to_node while
						 * we're checking.
						 */
						...
						if (afound) {
							...
							KDEBUG_RPS(
								0,
								"DVS: %s: did inode 0x%p file 0x%p new 0x%p\n",
								__FUNCTION__,
								ip, fp,
								another_fp);
							DVS_TRACEL(
					...
					/*
					 * Since an I/O request using this file
					 * pointer was issued, we should wait
					 * for the server to finish up rather
					 * than moving on at this point with the
					 * close.
					 *
					 * The server is using the remote ref
					 * file handle and if we continue on,
					 * our close that's about to happen
					 * could occur in the middle of the
					 * server's read operation so rather
					 * than trying to protect the server
					 * from failures by a bad file handle or
					 * etc, just wait for the message to
					 * come back on this end before closing
					 * this file instance.
					 *
					 * We could technically wait on either
					 * reads_outstanding or msgs_outstanding
					 * but msgs is a little safer for the
					 * server since we know that the server
					 * would be completely done with our
					 * file handle at that point.
					 */
					if (atomic_read(
						    &rq->msgs_outstanding)) {
						WAIT_FOR_MSGS(rq);
					...
					/*
					 * Come back to this one in hopes that
					 * we found another file instance
					 * further down the list that we can
					 * patch in here.
					 */
					rescan = required;
					continue;
	...
	list_for_each_safe (lp, tp, &iip->requests) {
		...
		case RPS_RQ_INVALID:
			if (mode == CLUP_Passive && rval != -EAGAIN) {
				/*
				 * Return a count (will look like an error which
				 * it is) of the ones that cannot be removed.
				 */
				rval++;
				break;
			}
			...
			/*
			 * In all of these cases, we need to make sure that
			 * there aren't messages arriving as we're trying to
			 * tear these down.
			 */
			refs_on_unlink = -1;
			for (i = 0; i < rq->msg_count; i++) {
				fip = &rq->freqs[i];
				freq = fip->freq; /* capture this before it can
						     go away */
				if ((ret = unlink_filerq(freq, fip->cookie,
							 fip->to_node,
							 UNLNK_Cleanup)) > 0) {
					DVS_BUG_ON(freq->u.iopagesrq.rq != rq);

					refs_on_unlink = atomic_dec_return(
						&rq->msgs_outstanding);

					(void)process_iovs(
						rq, &freq->u.iopagesrq.ipr,
						NULL, PRIOV_Cleanup);
					free_msg(freq);
				} else {
					/*
					 * We couldn't unlink it for one of
					 * several reasons.
					 *
					 *  - A message just came back and is
					 * being processed. That's only a
					 * problem if the request at this point
					 * still is marked as active.  If it's
					 * not, we'll return that we weren't
					 * successful and the upper layers will
					 * try again.
					 *
					 *  - EAGAIN was returned so let's back
					 * off and retry since failover is
					 * likely running and it's best to let
					 * it run vs. trying to cancel the
					 * request out from underneath it.
					 */
					if (ret == -EAGAIN) {
						rval = ret;
					} else {
						...
						rval = 1; /* being processed it
							     appears */

						/*
						 * If an outstanding write
						 * request made it to this point
						 * the writepages waiter must
						 * have been killed so clean up
						 * needs to be done.  If we
						 * couldn't unlink above but
						 * there are no more outstanding
						 * msgs then the last msg reply
						 * came back during the close
						 * and we can clean up now.
						 * Unless it is for a kthread,
						 * their teardown might lag
						 * behind the file close. Give
			...
			if (!refs_on_unlink) {
				/*
				 * We were able to unlink them all!  We know
				 * that the transport has completed the
				 * transaction or process_iovs() wouldn't have
				 * returned.
				 */
				KDEBUG_RPS(0,
					   "DVS: %s: cpu %d canceled rq 0x%p\n",
				...
				/*
				 * We couldn't unlink anything so somebody's
				 * closing this out. We'll check the requests
				 * again and this should have gone to expired by
				 * that time.  If after a few times it's still
				 * not expired, bail out and return the error
				 * upward.
				 */
				retry++;
			}
		      ...
		      int doing_readpage)
{
	 ...
	 * empty or the page is beyond the end of the file, no need to send a
	 * request to the server to find out the page doesn't exist.  Just zero
	 * the page and set it up for uwrite_begin.
	 */
	if (doing_readpage == DO_PRERD) {
		...
		if (page_offset >= ip->i_size) {
			KDEBUG_RPS(
				0,
				"DVS: %s: write to empty or write extend. page 0x%p, indx %lu"
				" off %Ld i_size %Ld\n",
				__FUNCTION__, single_page, single_page->index,
				page_offset, ip->i_size);
			zero_user(single_page, 0, PAGE_SIZE);
	 ...
	 * likely all with the same page list but that's not guaranteed.  All
	 * ranks will compute the extents but only 1 will win for each extent.
	 * A rank that wins one may not necessarily win others.
	 */
	 ...
	 * processing extents.  There can be multiple requests for each
	 * collection of pages so this needs to hang around until responses for
	 * all outstanding requests are received.
	 */
	atomic_set(&pagesd->ref_count, 1);
	atomic_set(&pagesd->state, RPS_PGSD_INUSE);

	/*
	 * Pages -- supplied by Linux -- already exist on a list, we want them
	 * in a sorted array so we can determine where the extents boundaries
	 * are.
	 */

	pagesd->pages =
		kmalloc_ssi(num_pages * sizeof(struct page *), GFP_KERNEL);
	if (!pagesd->pages) {
		rval = -ENOMEM;
		goto done;
	}
	...
	if (doing_readpage) {
		/*
		 * We're emulating readpage() so the arguments/conditions are
		 * quite different than how we'd be called from readpages() but
		 * we'll set everything up so that we can just jump on the
		 * readpages train as if it were already rolling with 1 extent.
		 */
...
extents_are_known:
	...
	/*
	 * OK, we know the exact number of extents needed at this point but we
	 * don't yet understand how many might require being split into 3
	 * extents ala case 4: below.  In that example, an extent that we've
	 * calculated here would find an existing request (in-flight) that is in
	 * the middle of our extent here, requiring that we split this extent
	 * into a new extent covering the beginning portion, a middle portion
	 * which matches the in-flight request and an extent for the tail
	 * portion.  In this case, each extent here worst-case would require 2
	 * so let's just allocate that many here where it's easy to do.
	 */
	if (extent_count * 2 > extents_alloc) {
		...
		new_extents = krealloc(
			extents, sizeof(extent_t) * extents_alloc, GFP_KERNEL);
		if (!new_extents) {
	...
	KDEBUG_RPS(
		0,
		"DVS: %s: cpu %d requested %d pages in %d extents for inode 0x%p (%s)\n",
		__FUNCTION__, raw_smp_processor_id(), page_count, extent_count,
		ip, fpname(fp));

	/*
	 * Go through each extent under the read_lock and see if we've got a
	 * match to an existing request.  Probability says that there is.  If
	 * we've got an extent that's only partially covered by a request, split
	 * it and add it to our extents assuming that somebody has already done
	 * the same and we haven't got that far yet.
	 *
	 * Once we've found something that's not covered, become a writer and
	 * recheck that we're still in the lead for that extent and continue to
	 * hold the write lock until we find something that's covered by an
	 * existing request.
	 */
	...
	next_extent:
		...
		rp_state = rp_unset;
		KDEBUG_RPS(0,
			   "DVS: %s: cpu %d extent %d(of %d) for inode 0x%p "
			   "off %Ld(page %Ld) len %ld(%d)\n",
			   __FUNCTION__, raw_smp_processor_id(), start,
			   extent_count, ip, offset, base_indx, count,
			   ep->count);
		DVS_TRACEL("RPSprExt", ip, offset, count, start, extent_count);

	find_check:
		list_for_each_safe (lp, tp, &iip->requests) { /* we delete
								 expired ones */
			size_t diff;
			pages_request_t *lrq;
			...
			/*
			 * Skip/clean up expired requests so that the proxy
			 * threads don't have to slow things down by grabbing
			 * the write lock.
			 */
			...
			case RPS_RQ_EXPIRING:
			case RPS_RQ_INVALID: /* handle it at the close of the
						file */
				continue;
			...
			/*
			 * First off, let's see if they touch in some fashion.
			 * If so, we would hope that this extent is completely
			 * contained in an outstanding request but it's possible
			 * we may have to split an extent into covered vs. not
			 * covered piece.  There should be at most two
			 * not-covered portions.
			 */
			if ((count + lrq->length) >
				    (diff = ((lrq->offset + lrq->length) -
					     offset)) &&
			    (diff > 0)) {
				/*
				 * They touch! -- four possibilities:
				 *   1) This extent starts prior to the request
				 * but overlaps, so we need to create a new
				 * extent for the initial portion. 2) This
				 * extent is completely contained by the
				 * request.  This is easy because we're done
				 * with this one then. 3) This extent overlaps
				 * on the end of the request and is not covered
				 * for a portion beyond, so we need to create a
				 *      new extent for the tail portion.
				 *   4) This extent completely spans the request
				 * so we need to create two extents, one for the
				 * initial portion and one for the tail.
				 *
				 * We'll start with the most common first.  If
				 * there's nothing to do we'll move to the next
				 * extent but otherwise need to reduce this
				 * extent by the amount that touches and keep
				 * searching.
				 */
				int original_extent = start;

				/*
				 * If this extent touches one that we created
				 * something went wrong in either extent
				 * creation or extent processing.
				 */
				DVS_BUG_ON(pagesd == lrq->pagesd);

				if (offset >= lrq->offset) {
					if ((offset + count) <=
					    (lrq->offset + lrq->length)) {
						/* case 2 -- easy! */
						/*
						 * Something's wrong.  We were
						 * supplied a locked page that
						 * the kernel added to the cache
						 * for us and yet there's an
						 * outstanding request for that
						 * same page which would also
						 * have been in the cache at
						 * that point.
						 */
						DVS_BUG_ON(doing_readpage);

						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 2 extent %d for inode 0x%p "
						...
						if (sema_state == writer) {
							...
							KDEBUG_RPS(
								0,
								"DVS: %s: cpu %d backing off as writer on 0x%p\n",
								__FUNCTION__,
								raw_smp_processor_id(),
								lrq);
						}

						goto next_extent; /* nothing to
								     do with
								     this
								     extent,
								     it's
						...
						/* case 3 -- not covered off the
						 * back - shorten the front */
						diff /= PAGE_SIZE; /* diff still
								      has what
								      we want */
						ep->indx += diff;
						ep->count -= diff;
						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 3 extent %d for inode 0x%p off"
						...
						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 3 extent %d inode 0x%p "
					...
					int request_pages =
						lrq->length / PAGE_SIZE;

					diff = ((offset + count) -
						lrq->offset) /
					       PAGE_SIZE;
					ep->count -= diff; /* shorten the end in
							      either case */
					...
					if (diff > request_pages) {
						...
						/*
						 * case 4:
						 * Both ends are not covered -
						 * yuk.  OK, shorten this extent
						 * by the full range of the
						 * request and add a new extent
						 * for the remainder off the
						 * end.
						 *
						 * The new extent will be out of
						 * offset order but that's OK.
						 */

						/* TODO: remove at some point
						 * now allocating 2x above */
						DVS_BUG_ON(extent_count ==
							   extents_alloc);
						...
						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 4 extent %d for inode 0x%p "
							"off %Ld len %ld request 0x%p Roff %Ld Rlen %ld\n",
							__FUNCTION__,
							raw_smp_processor_id(),
							original_extent, ip,
						...
						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 4 extent %d inode 0x%p "
						...
						/* case 1 -- we've already
						 * shortened the end of the
						 * extent */

						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 1 extent %d for inode 0x%p "
							"off %Ld len %ld request 0x%p Roff %Ld Rlen %ld\n",
							__FUNCTION__,
							raw_smp_processor_id(),
							original_extent, ip,
						...
						KDEBUG_RPS(
							0,
							"DVS: %s: cpu %d case 1 extent %d inode 0x%p "
				...
				/*
				 * OK, we just modified the extent we started
				 * with, let's just start the whole thing over
				 * since we could now have been covered by one
				 * that we passed up earlier.  Reset the extent
				 * pointer so we see this one again on the next
				 * iteration.
				 */
				if (decreasing_order) {
					start++;
				...
				KDEBUG_RPS(
					0,
					"DVS: %s: cpu %d case 1,3,4 extent %d for inode 0x%p "
					"request 0x%p restart start to %d\n",
					__FUNCTION__, raw_smp_processor_id(),
					original_extent, ip, lrq, start);
				goto next_extent; /* process this modified one
						     again */
			} /* the two requests touch */
		} /* foreach request list */

		/*
		 * If we made it to this point, we didn't find an outstanding
		 * request that matched our range.
		 */
		if (sema_state == writer) {
			pages_request_t *rq;
			int *page_status;
			...
			page_status = kmalloc_ssi(sizeof(int) * ep->count,
						  GFP_KERNEL);
			if (!page_status) {
				rval = -ENOMEM;
				goto done;
			...
			/*
			 * OK, it's the real deal - we won -- maybe!  Check to
			 * make sure we can add it to the page cache first.  If
			 * that's the case, we get to keep the write lock since
			 * it appears that we're in the lead!
			 */
			...
			for (i = 0; i < ep->count; i++) {
				...
				ret = (doing_readpage) ?
					      0 :
					      add_to_page_cache(tp, mapping,
								tp->index,
								GFP_KERNEL);
				if (ret) {
					error_count++;
					page_status[i] = ret; /* need to know
								 which ones we
								 added */
					pending_rval = ret;
				} else {
					if (!doing_readpage) {
						list_del(&tp->lru); /* page_list
								       threads
								       through
								       lru */
			...
			if (error_count) {
				/*
				 * OK, we're obviously quite late to the party
				 * since it's already been added to the cache.
				 * Back out as a writer. The request likely came
				 * and went from the list already.
				 */
				if (error_count != ep->count) {
					/*
					 * Some apparently made it.  Walk the
					 * list and find the ones that we
					 * successfully added to the page cache
					 * and mark them in error.  They'll come
					 * back in through ureadpage() if
					 * needed.
					 */
					for (i = 0; i < ep->count; i++) {
						if (!page_status[i]) {
							...
							KDEBUG_RPS(
								0,
								"DVS: %s: cpu %d page %ld added to existing "
								"range for %s\n",
								__FUNCTION__,
								raw_smp_processor_id(),
								tp->index,
				...
				KDEBUG_RPS(
					0,
					"DVS: %s: cpu %d backing writer off on extent %d for "
			...
			/*
			 * Continue using the default number of freqs for
			 * readpage requests. May need to be revisited if
			 * readpages kernel interface or blocksize handling
			 * changes.
			 */
			...
			/* freqs, rma_handle, vmap_addr filled in by
			 * process_extent() */
			rq->ip = ip;
			rq->fp = fp; /* fast way to know which file initiated
					this request */
			rq->offset = offset;
			rq->length = count;
			...
			atomic_inc(&pagesd->ref_count); /* this pages request
							   uses pagesd */
			atomic_set(&rq->state, RPS_RQ_ACTIVE);

			rq->rq_flags |=
				(doing_readpage) ? RPS_RQ_FLAGS_READPAGE : 0;

			/* notify process_iovs this is a pre-read for a page
			 * cache write */
			if (doing_readpage == DO_PRERD) {
			...
			KDEBUG_RPS(
				0,
				"DVS: %s: cpu %d won extent %d for inode 0x%p"
				"off %Ld(page %Ld) len %ld\n",
				__FUNCTION__, raw_smp_processor_id(), start, ip,
				offset, base_indx, count);
			KDEBUG_RPS(
				0,
				"DVS: %s: cpu %d extent %d for inode 0x%p added request 0x%p"
				" pagesd 0x%p ep_indx %d ep_count %d\n",
				__FUNCTION__, raw_smp_processor_id(), start, ip,
				rq, pagesd, ep->indx, ep->count);
			DVS_TRACEL("RPSWRq", ip, offset, count, rq,
				   extent_count);

			rp_state++; /* starting extent read */
			if ((rval = process_extent(fp, rq)) < 0) {
				/*
				 * If this had an error it would have cleaned up
				 * as much as possible.  For now, let's bail out
				 * and fail the whole call as opposed to
				 * thinking that perhaps the next extent (if
				 * there is one) might work OK.  The likelyhood
				 * is it won't.
				 */
				goto done;
			}
			...
			/*
			 * If there was a reliable way to check the pages to see
			 * if they're already in the cache before we become a
			 * writer that'd be better. We could be way late and the
			 * request has been processed, deleted with the pages
			 * already being in the cache.
			 */
			DVS_BUG_ON(sema_state != reader);
			up_read(&iip->requests_sem);
			...
			KDEBUG_RPS(
				0,
				"DVS: %s: cpu %d now writer for extent %d inode 0x%p"
				"off %Ld(page %Ld) len %ld\n",
				__FUNCTION__, raw_smp_processor_id(), start, ip,
				offset, base_indx, count);
			goto find_check;
	...
	if (rval) {
		if (doing_readpage) {
			/*
			 * The supplied page is always placed in the page cache
			 * by the kernel and released after we're done.  Things
			 * may not have been allocated properly so use the
			 * initial variables to process the page here.
			 */
			...
			if (rp_state == rp_in_cache) {
				/*
				 * It's in the cache but we didn't make it to
				 * the read_extent() call.  That means we need
				 * to error every extent page. read_extent()
				 * would have otherwise taken care of erroring
				 * the pages related to the actual error.
				 */
				loff_t start_index =
					pagesd->pages[ep->indx]->index;

				DVS_TRACEL("RPSrPCdE", ip, start_index,
					   ep->count, rval, 0);
				KDEBUG_RPS(
					0,
					"DVS: %s: cpu %d erroring pages %Ld-%Ld of %s on exit\n",
					__FUNCTION__, raw_smp_processor_id(),
					start_index,
					start_index + ep->count - 1,
					fpname(fp));

				error_page_range(&pagesd->pages[ep->indx],
						 ep->count, 1, 0);
			}
		}
	}
	...
	if (sema_state == writer) {
		...
		KDEBUG_RPS(0,
			   "DVS: %s: cpu %d ip 0x%p unlocking as writer done\n",
	 ...
	 * the requests chain (being valid or invalid) or possibly got rid of
	 * it.
	 */
	if (extents) {
	...
	if (pagesd && !atomic_dec_return(&pagesd->ref_count)) {
		/*
		 * We either didn't use pagesd or we did and the messages all
		 * came back and were processed before we could leave this
		 * routine.
		 */
		if (pagesd->pages) {
			kfree_ssi(pagesd->pages);
			...
			struct page **pagep, void **fsdata)
{
	...
	KDEBUG_OFC(
		0,
		"DVS: %s: fp 0x%p map 0x%p pos %lld len %u index %lu flags %u\n",
...
static int dvs_fdatawrite_nr_nowait(struct address_space *mapping, int count)
{
	...
	if (mapping->a_ops->writepages)
		...
		DVS_BUG(); /* writepages should always be set in DVS */
		      ...
		      struct page *pagep, void *fsdata)
{
	 ...
	 */
	dvsdebug_stat_update(INODE_ICSB(inode)->stats, DVSSYS_STAT_CLIENT_LEN,
			     VFS_OP_AIO_WRITE, len);
	dvsdebug_stat_update(INODE_ICSB(inode)->stats, DVSSYS_STAT_CLIENT_OFF,
...
static loff_t ulseek_stats(struct file *fp, loff_t off, int op)
{
	loff_t ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;

	ret = ulseek(fp, off, op);
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_LLSEEK, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_LLSEEK,
			     elapsed_jiffies);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, 9)
static ssize_t do_sync_read_stats(struct file *fp, char __user *buf, size_t len,
				  loff_t *ppos)
{
	ssize_t ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_READ, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_READ,
			     elapsed_jiffies);

	return ret;
}

static ssize_t uread_stats(struct kiocb *iocb, const struct iovec *iov,
			   unsigned long nr_segs, loff_t pos)
{
	ssize_t ret;
	struct dvsdebug_stat *stats = FILE_ICSB(iocb->ki_filp)->stats;
	unsigned long elapsed_jiffies = jiffies;

	ret = uread(iocb, iov, nr_segs, pos);
	elapsed_jiffies = jiffies - elapsed_jiffies;
	KDEBUG_OFC(0, "DVS: uread_stats: uread returns %ld\n", ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_AIO_READ, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_AIO_READ,
			     elapsed_jiffies);

	return ret;
}

static ssize_t do_sync_write_stats(struct file *fp, const char __user *buf,
				   size_t len, loff_t *ppos)
{
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_WRITE, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_WRITE,
			     elapsed_jiffies);

	return ret;
}

static ssize_t uwrite_stats(struct kiocb *iocb, const struct iovec *iov,
			    unsigned long nr_segs, loff_t pos)
{
	ssize_t ret;
	struct dvsdebug_stat *stats = FILE_ICSB(iocb->ki_filp)->stats;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_AIO_WRITE, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_AIO_WRITE,
			     elapsed_jiffies);
...
static int ureaddir_stats(struct file *fp, struct dir_context *ctx)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_READDIR, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_READDIR,
			     elapsed_jiffies);

	return ret;
}

static long uioctl_stats(struct file *fp, unsigned int cmd, unsigned long arg)
{
	long ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_UNLOCKED_IOCTL,
			     ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME,
			     VFS_OP_UNLOCKED_IOCTL, elapsed_jiffies);

	return ret;
}

static int ummap_stats(struct file *fp, struct vm_area_struct *vma)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	ret = ummap(fp, vma);
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_MMAP, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_MMAP,
			     elapsed_jiffies);

	return ret;
}

static int uopen_stats(struct inode *ip, struct file *fp)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	ret = uopen(ip, fp, 0);
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_OPEN, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_OPEN,
			     elapsed_jiffies);

	return ret;
}

static int uflush_stats(struct file *fp, fl_owner_t id)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	ret = uflush(fp, id);
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_FLUSH, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_FLUSH,
			     elapsed_jiffies);

	return ret;
}

static int urelease_stats(struct inode *ip, struct file *fp)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_RELEASE, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_RELEASE,
			     elapsed_jiffies);

	return ret;
}

static int ufsync_stats(struct file *fp, loff_t off1, loff_t off2, int datasync)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_FSYNC, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_FSYNC,
			     elapsed_jiffies);

	return ret;
}

static int ufasync_stats(int arg1, struct file *fp, int arg3)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_FASYNC, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_FASYNC,
			     elapsed_jiffies);

	return ret;
}

static int ulock_stats(struct file *fp, int cmd, struct file_lock *fl)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	ret = ulock(fp, cmd, fl);
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_LOCK, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_LOCK,
			     elapsed_jiffies);

	return ret;
}

static int uflock_stats(struct file *fp, int cmd, struct file_lock *fl)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_FLOCK, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_FLOCK,
			     elapsed_jiffies);

	return ret;
}

static int uwritepage_stats(struct page *pagep, struct writeback_control *wbc)
{
	int ret;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_WRITEPAGE, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_WRITEPAGE,
			     elapsed_jiffies);

	return ret;
}

static int uwritepages_stats(struct address_space *mapping,
			     struct writeback_control *wbc)
{
	int ret;
	struct dvsdebug_stat *stats = NULL;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_WRITEPAGES, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_WRITEPAGES,
			     elapsed_jiffies);

	return ret;
}

static int ureadpage_stats(struct file *fp, struct page *pagep)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	unsigned long elapsed_jiffies = jiffies;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_READPAGE, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_READPAGE,
			     elapsed_jiffies);

	return ret;
}

static int ureadpages_stats(struct file *fp, struct address_space *mapping,
			    struct list_head *page_list, unsigned num_pages)
{
	int ret;
	struct dvsdebug_stat *stats = FILE_ICSB(fp)->stats;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_READPAGES, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_READPAGES,
			     elapsed_jiffies);

	return ret;
}

static int uwrite_begin_stats(struct file *fp, struct address_space *mapping,
			      loff_t pos, unsigned len, unsigned flags,
			      struct page **pagep, void **fsdata)
{
	int ret;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_WRITE_BEGIN, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_WRITE_BEGIN,
			     elapsed_jiffies);

	return ret;
}

static int uwrite_end_stats(struct file *fp, struct address_space *mapping,
			    loff_t pos, unsigned len, unsigned copied,
			    struct page *pagep, void *fsdata)
{
	int ret;
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_WRITE_END, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_WRITE_END,
			     elapsed_jiffies);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, 9)
static ssize_t udirect_IO_stats(int rw, struct kiocb *iocb,
				...
				unsigned long nr_segs)
{
	...
	elapsed_jiffies = jiffies - elapsed_jiffies;
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER, VFS_OP_DIRECT_IO, ret);
	dvsdebug_stat_update(stats, DVSSYS_STAT_OPER_TIME, VFS_OP_DIRECT_IO,
			     elapsed_jiffies);

	return ret;
}
#else
static ssize_t udirect_IO_stats(struct kiocb *iocb, struct iov_iter *iter)
{
	ssize_t ret;
	struct dvsdebug_stat *stats = FILE_ICSB(iocb->ki_filp)->stats;
	unsigned long elapsed_jiffies = jiffies;
 ...
 *
 * .read
 *   do_sync_read_stats +DVSSYS_STAT_OPER,VFS_OP_READ
 *     do_sync_read
 *       filp->f_op->aio_read == .aio_read
 *         do_sync_read_stats +DVSSYS_STAT_OPER,VFS_OP_AIO_READ
 *           uread +DVSSYS_STAT_CLIENT_LEN,VFS_OP_AIO_READ
 *                 +DVSSYS_STAT_CLIENT_OFF,VFS_OP_AIO_READ
 ...
 *
 * .write
 *   do_sync_write_stats +DVSSYS_STAT_OPER,VFS_OP_WRITE
 *     do_sync_write
 *       filp->f_op->aio_write == .aio_write
 *         uwrite_stats +DVSSYS_STAT_OPER,VFS_OP_AIO_WRITE
 *           uwrite +DVSSYS_STAT_CLIENT_LEN,VFS_OP_AIO_WRITE
 *                  +DVSSYS_STAT_CLIENT_OFF,VFS_OP_AIO_WRITE
 *             uwrite2
...
static int __init init_dvs(void)
{
	int i;
	int rval;

	extern void (*dvs_close_quiesced_files_func)(struct quiesced_dir *
						     qdir);

	dvs_task_work_run_func = (void *)kallsyms_lookup_name("task_work_run");
	dvs_tasklist_lock = (rwlock_t *)kallsyms_lookup_name("tasklist_lock");

	if (initialize_syscall_linkage() < 0) {
		printk(KERN_ERR "DVS: Could not find sys_call_table!!! "
				"Exiting!\n");
		return -EFAULT;
	}
	...
	if (dvsof_concurrent_writes != -1) {
		if (dvsof_concurrent_writes < 0) {
			printk(KERN_ERR "DVS: %s: Error: Invalid value %d for "
			       ...
			       __func__, dvsof_concurrent_writes);
			return -EINVAL;
		}
	...
	if (dvsof_concurrent_reads != -1) {
		if (dvsof_concurrent_reads < 0) {
			printk(KERN_ERR "DVS: %s: Error: Invalid value %d for "
			       ...
			       __func__, dvsof_concurrent_reads);
			return -EINVAL;
		}
	...
	for (i = 0; i < numoptions; i++) {
		...
		KDEBUG_OFC(0, "DVS: %s: option %d len %d name %s\n",
			   __FUNCTION__, i, optionlist[i].opt_len,
			   optionlist[i].opt_name);
	}
...
static void __exit exit_dvs(void)
{
