...
unsigned int *
sync_period_get(void)
{
	return &sync_period_secs;
}

unsigned int *
sync_timeout_get(void)
{
	return &sync_dirty_timeout_secs;
}

unsigned int *
sync_threads_get(void)
{
	return &sync_num_threads;
}

void
sync_stats_print(struct seq_file *m)
{
...
int
sync_stats_control(unsigned int control)
{
	if (!control) {
...
int
sync_timeout_update(unsigned int timeout)
{
...
int
sync_threads_update(unsigned int threads)
{
...
int
sync_period_update(unsigned int period)
{
...
static void
sync_wakeup_thread(unsigned long data)
{
	up(&sync_sema);
}

static int
sync_thread(void *data)
{
...
sync_init(void)
{
	...
	if (sync_hash_size && (inode_hashtable =
	                       kmalloc_ssi(sizeof(struct inode_hashlist) *
	                       sync_hash_size, GFP_KERNEL)) == NULL) {
	...
	if ((fsync_threads = kmalloc_ssi(sizeof(struct fsync_thread_info) *
	                         SYNC_MAX_FSYNC_THREADS, GFP_KERNEL)) ==  NULL) {
	...
	for (i = 0; i < sync_num_threads; i++) {
		if (sync_add_fsync_thread(&fsync_threads[i], i)) {
			if (i != 0) {
				...
				DVS_LOGP("Sync: could not start fsync threads\n");
				return -EINVAL;
			}
		}
	}
...
int
sync_is_inode_dirty(struct remote_ref *rr)
{
	struct inode_ref *ir;
...
static int
sync_add_fsync_thread(struct fsync_thread_info *fsync_info, int num)
{
	struct task_struct *task;
...
static void
sync_remove_fsync_thread(struct fsync_thread_info *fsync_info)
{
	if (fsync_info->run != 1)
		return;
...
int
fsync_inode_ref(struct inode_ref *ir, struct remote_ref *rr)
{
	...
	if (vfs_fsync(rr->fp, rr->fp->f_path.dentry, 0) == 0) {
#else
...
int
sync_add_inode_ref(struct remote_ref *rr)
{
	struct inode_hashlist *inode_hashentry;
	...
	inode_hashentry = &inode_hashtable[hash_fnv_1a(ino) % sync_hash_size];

	read_lock(&inode_hashentry->lock);
	list_for_each_entry(current_ref,
	                    &inode_hashentry->inode_list, inode_list) {
	...
	if ((new_ref = kmalloc_ssi(sizeof(struct inode_ref),
	                           GFP_KERNEL)) == NULL)
		return -ENOMEM;
...
void
sync_remove_inode_ref(struct remote_ref *rr)
{
	struct inode_ref *ir;
	struct inode_hashlist *inode_hashentry;
...
void
sync_server_data_written(struct remote_ref *rr)
{
	if (!sync_hash_size)
		return;
...
void
sync_server_bulk_update(unsigned long *inodes, unsigned long *sync_times, int size)
{
	int i;
	...
	for (i = 0; i < size; i++) {
		...
		if (sync_hash_size)
			inode_hashentry = &inode_hashtable[hash_fnv_1a(inodes[i])
			                                   % sync_hash_size];
		else
...
static void
sync_client_bulk_update(struct per_node *pna, struct ssi_server_info *server_info, struct remote_file **dirty_rf)
{
	...
	list_for_each_entry(rf, &server_info->rf_list, list) {
		for (i = finished_inodes;
		            i < pna->request->u.syncupdaterq.size; i++) {
			if (rf == dirty_rf[i]) {
				if (i == finished_inodes)
					finished_inodes++;
				sync_client_sync_update(pna->reply->u.syncupdaterp.sync_times[i],
...
static int
sync_client_force_update(int *servers, int num_servers)
{
	struct per_node *pna;
	...
	for (nord = 0; nord < num_servers; nord++) {
		...
		rqsize = sizeof(struct file_request) + (sizeof(unsigned long) * open_files);
		rpsize = sizeof(struct file_reply) + (sizeof(unsigned long) * open_files);
		...
		spin_lock(&server_info->lock);
		list_for_each_entry(rf, &server_info->rf_list, list) {
			if (rf->last_write > rf->last_sync && rf->last_write <
		...
		rqsize = sizeof(struct file_request) + (sizeof(unsigned long) * dirty_count);
		rpsize = sizeof(struct file_reply) + (sizeof(unsigned long) * dirty_count);
		...
		log_request(filerq->request, NULL, NULL, NULL, 1,
			    servers[nord], jiffies - elapsed_jiffies);
	...
	for (nord = 0; nord < num_servers; nord++) {
		if (pna[nord].sent) {
			if (!wait_for_async_request_stats(NULL,
							  pna[nord].request))
				sync_client_bulk_update(&pna[nord],
				            node_map[servers[nord]].server_info,
				            dirty_rf[nord]);
		}
		kfree(pna[nord].request);
		kfree(pna[nord].reply);
		kfree(dirty_rf[nord]);
	}
...
sync_client_do_timer(void)
{
	...
	list_for_each_entry(server_info, &sync_client_server_list, list) {
		if (server_info->flags & SYNC_SERVER_DOWN)
			continue;
		if (!servers && (servers = (int *)
		                 kmalloc_ssi(sync_client_num_servers * 
		                 sizeof(int), GFP_ATOMIC)) == NULL) {
			ret = -ENOMEM;
			break;
		}
 ...
 */
void
sync_client_sync_update(long delta, long start, struct remote_file *rf)
{
	long sync_time;
...
void
sync_client_data_written(struct remote_file *rf)
{
...
static void
sync_client_mark_server_down(int node)
{
	read_lock(&sync_client_server_lock);
...
static void
sync_client_remove_server(int node)
{
	struct ssi_server_info *server_info;
...
int
sync_client_add_server(int node, int flags,
                       struct incore_upfs_super_block *icsb)
{
	...
	if (node_map[node].server_info == NULL) {
		if ((server_info = (struct ssi_server_info *)
		             kmalloc_ssi(sizeof(struct ssi_server_info),
		             GFP_ATOMIC)) == NULL) {
...
int
sync_client_check_dirty(int node, struct file *fp)
{
