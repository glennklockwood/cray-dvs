...
#define MSGQ_HASH(key, inmsgq) (inmsgq->single_msg_queue ? 0 : (hash_long(key, inmsgq->htb->numbits) + 1))

/* controls cleanup of inactive incoming message queues */
#define DVSIPC_INMSGQ_TIMEOUT (360 * HZ)

#define DVSIPC_INMSGQ_FREEPOOL_MIN dvs_cur_config_params->msgq_init_free_qhdrs/4
...
#define MSGQ_INC_QLEN(qhdr) {\
    ...
    qhdr == qhdr->inmsgq->anykey_hdr ? \
            ...
            atomic_inc(&(qhdr->inmsgq->htb->buckets[MSGQ_HASH(qhdr->htb_key, qhdr->inmsgq)].qlen));}
#define MSGQ_DEC_QLEN(qhdr) {\
    atomic_dec(&qhdr->inmsgq->incomingq_len); \
    qhdr == qhdr->inmsgq->anykey_hdr ? \
            atomic_dec(&qhdr->inmsgq->htb->buckets[0].qlen) : \
            atomic_dec(&(qhdr->inmsgq->htb->buckets[MSGQ_HASH(qhdr->htb_key, qhdr->inmsgq)].qlen));}

#define DEBUG_QHEADER(str, qhdr) \
    if (qhdr == NULL) { \
        KDEBUG_IPC(0, "DVS: %s: %s: qhdr (null)\n", __FUNCTION__, str); \
    } else { \
        KDEBUG_IPC(0, "DVS: %s: %s: qhdr 0x%p, key %ld, qlist 0x%p:0x%p, " \
                "msghead 0x%p:0x%p, chain 0x%p:0x%p\n", \
               __FUNCTION__, str, qhdr, (long int)qhdr->htb_key, \
               qhdr->prev_queue, qhdr->next_queue, \
               qhdr->msgq_tail, qhdr->msgq_head, \
               qhdr->prev_htb_chain, qhdr->next_htb_chain); \
    }

extern int dvsipc_start_incoming_msgq(struct dvsipc_incoming_msgq *inmsgq);
struct dvsipc_incoming_msgq *dvsipc_create_incoming_msgq(const char *thread_name,
                                                         unsigned int init_free_qhdrs,
                                                         unsigned int max_free_qhdrs,
                                                         int single_msg_queue,
...
extern void msgq_shrink_freepool(int count, struct dvsipc_incoming_msgq *inmsgq);
extern void msgq_grow_freepool(int count, struct dvsipc_incoming_msgq *inmsgq);
extern struct msgq_qheader *msgq_alloc_qheader(struct dvsipc_incoming_msgq *inmsgq);
extern struct msgq_qheader *msgq_get_next_queue(struct dvsipc_incoming_msgq *inmsgq);

extern msgq_key_t dvsipc_get_queue_key(struct dvsipc_incoming_msgq *inmsgq, struct usiipc *msg);

int dvsipc_add_msg_to_qheader(struct dvsipc_incoming_msgq *inmsgq, struct usiipc *msg);
