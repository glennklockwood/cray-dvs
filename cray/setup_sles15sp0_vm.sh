...
zypper ar --no-gpgcheck https://download.opensuse.org/repositories/benchmark/SLE_15 benchmark
zypper ar --no-gpgcheck http://srd-jenkins.us.cray.com/job/ci-dvs-lustre/job/feature%2Fshared%2Fshasta/lastSuccessfulBuild/artifact/sles15sp0 lustre
kernel_rpm_version=$(rpm -q --queryformat '%{version}-%{release}\n' kernel-default)
zypper -n in gcc make automake autoconf bc patch git kernel-source=${kernel_rpm_version} kernel-default-devel=${kernel_rpm_version} kernel-syms=${kernel_rpm_version}
